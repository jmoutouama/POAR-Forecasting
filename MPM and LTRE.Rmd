...
# Estimating the mean and standard deviation to use them in the two sex matrix function
```{r,eval=FALSE}
# Survival
poar.clim_seasonal %>% 
  subset( tillerN_t0 > 0 )%>%
...
expand.grid(grow[,1],grow[,2])
lambda_grow_mean<-lambda_grow_mean_2sex<-SR_grow_mean<-OSR_grow_mean<-c()
grow<-matrix(c(pptgrow_seq_extend,tempgrow_seq_extend,rep(mean(poar.clim_seasonal$pptdorm),10),rep(mean(poar.clim_seasonal$tempdorm),10)),ncol=4)

# grow<-data.frame(pptgrow_seq_extend,tempgrow_seq_extend)
...
dorm<-matrix(c(pptdorm_seq_extend,tempdorm_seq_extend,rep(mean(poar.clim_seasonal$pptgrow),10),rep(mean(poar.clim_seasonal$tempgrow),10)),ncol=4)
ssd<-matrix(NA,(F_params$max_size+1)*2,length(pptdorm_seq_extend))
max_yrs <- 10

for (i in 1:nrow(dorm)) {
   #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,

...
```{r}
poar_allsites.clim_season <- left_join(x = poar_allsites_2015_2016 ,y =poar_climseason,by=c("site","census.year")) # merge the demographic data with climatic data for each site
...
Fdom_meanclim_mat <- megamatrix_delay( F_params=F_params,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           pptgrow=mean(poar.clim_seasonal$pptgrow),
                           tempgrow=mean(poar.clim_seasonal$tempgrow),
                           pptdorm=mean(poar.clim_seasonal$pptdorm),
                           tempdorm=mean(poar.clim_seasonal$tempdorm),
                           rfx=rfx_fun())$MEGAmat
popbio::lambda(Fdom_meanclim_mat) ## this is unrealistically high

twosex_meanclim_mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=T,
                           grow_perturb=0,
                           surv_perturb=0,

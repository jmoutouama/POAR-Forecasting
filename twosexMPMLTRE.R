...
sx<-function(x,params,pptgrow,pptdorm,tempgrow,tempdorm,rfx,surv_perturb=0){
  ...
  tempdorm_scaled_s=(tempdorm-27.36059)/1.44083
...
gxy<-function(x,y,params,pptgrow,pptdorm,tempgrow,tempdorm,rfx,grow_perturb=0){
   # xb<-pmin(x,params$max_size) # any predicted plants larger than max size will set to be max size
    pptgrow_scaled_g=(pptgrow-706.8709)/339.4783
    pptdorm_scaled_g=(pptdorm-366.5321)/140.0161
    tempgrow_scaled_g=(tempgrow-15.2168)/3.149584
    tempdorm_scaled_g=(tempdorm- 27.34905)/1.453832
...
# sum(gxy(15,1:125,params=F_params,pptgrow,pptdorm,tempgrow,tempdorm,rfx))
...
# y=1:40
# plot(pxy(x=y,y=y,params=F_params,pptgrow,pptdorm,tempgrow,tempdorm,rfx=rfx,surv_perturb=0,grow_perturb=0))

...
poar_surv_binned <- poar.surv %>% 
  mutate(pptgrow_bin = as.integer(cut_number(z_ppt_t1_grow,pptgrow_bin_num))) %>% 
  group_by(site,census.year,Sex,pptgrow_bin) %>% 
...
surv_mean_sizes <- poar_surv_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(size = mean(mean_size))
surv_mean_pptgrow <- poar_surv_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptgrow = mean(pptgrow))
surv_mean_tempgrow <- poar_surv_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempgrow = mean(tempgrow))
surv_mean_pptdorm <- poar_surv_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptdorm = mean(pptdorm))
surv_mean_tempdorm <- poar_surv_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempdorm = mean(tempdorm))
...
poar_grow_binned <- poar.grow %>% 
  mutate(pptgrow_bin = as.integer(cut_number(log_size_t0,pptgrow_bin_num))) %>% 
  group_by(site,census.year,Sex,pptgrow_bin) %>% 
  summarise(mean_size = mean(log_size_t0),
            ...
            tempdorm = unique(z_temp_t1_dorm),
            bin_n = n())
grow_mean_sizes <- poar_grow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(size = mean(mean_size))
grow_mean_pptgrow <- poar_grow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptgrow = mean(pptgrow))
grow_mean_tempgrow <- poar_grow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempgrow = mean(tempgrow))
grow_mean_pptdorm <- poar_grow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptdorm = mean(pptdorm))
grow_mean_tempdorm <- poar_grow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempdorm = mean(tempdorm))
...
poar_flow_binned <- poar.flow %>% 
  mutate(pptgrow_bin = as.integer(cut_number(log_size_t1,pptgrow_bin_num))) %>% 
  group_by(site,census.year,Sex,pptgrow_bin) %>% 
  summarise(mean_size = mean(log_size_t1),
            ...
            tempdorm = unique(z_temp_t1_dorm),
            bin_n = n())
flow_mean_sizes <- poar_flow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(size = mean(mean_size))
flow_mean_pptgrow <- poar_flow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptgrow = mean(pptgrow))
flow_mean_tempgrow <- poar_flow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempgrow = mean(tempgrow))
flow_mean_pptdorm <- poar_flow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptdorm = mean(pptdorm))
flow_mean_tempdorm <- poar_flow_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempdorm = mean(tempdorm))
...
panic_mean_sizes <- poar_panic_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(size = mean(mean_size))
panic_mean_pptgrow <- poar_panic_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptgrow = mean(pptgrow))
panic_mean_tempgrow <- poar_panic_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempgrow = mean(tempgrow))
panic_mean_pptdorm <- poar_panic_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(pptdorm = mean(pptdorm))
panic_mean_tempdorm <- poar_panic_binned %>% group_by(Sex,pptgrow_bin) %>% summarise(tempdorm = mean(tempdorm))
```
...
surv_sex_diff_post_tempgrow <- grow_sex_diff_post_tempgrow <- flow_sex_diff_post_tempgrow <- panic_sex_diff_post_tempgrow <- array(NA,dim=c(pptgrow_bin_num,length(tempgrow_seq),n_post_draws))
for(p in 1:n_post_draws){
  for(i in 1:pptgrow_bin_num){
    s=1;      
    fem_s <-invlogit((surv_coef$b0_s[post_draws[p]]) +
                        (surv_coef$bsize_s[post_draws[p]]) * surv_mean_sizes$size[surv_mean_sizes$Sex==s] +
                        (surv_coef$bsizesex_s[post_draws[p]]) * surv_mean_sizes$size[surv_mean_sizes$Sex==s] * (s-1) +
                        (surv_coef$bsex_s[post_draws[p]]) * (s-1) +
                        (surv_coef$bpptgrow_s[post_draws[p]]) * surv_mean_pptgrow$pptgrow[surv_mean_pptgrow$Sex==s & surv_mean_pptgrow$pptgrow_bin==i]  +
                        ...
                        (surv_coef$bpptgrowsex_s[post_draws[p]]) * surv_mean_pptgrow$pptgrow[surv_mean_pptgrow$Sex==s & surv_mean_pptgrow$pptgrow_bin==i] * (s-1) +
                        ...
                        (surv_coef$btempgrowpptgrow_s[post_draws[p]]) * tempgrow_seq * surv_mean_pptgrow$pptgrow[surv_mean_pptgrow$Sex==s & surv_mean_pptgrow$pptgrow_bin==i] +
                        ...
                        (surv_coef$bpptgrow2_s[post_draws[p]]) * (surv_mean_pptgrow$pptgrow[surv_mean_pptgrow$Sex==s & surv_mean_pptgrow$pptgrow_bin==i])^2 +
                        ...
                        (surv_coef$bpptgrow2sex_s[post_draws[p]]) * (surv_mean_pptgrow$pptgrow[surv_mean_pptgrow$Sex==s & surv_mean_pptgrow$pptgrow_bin==i])^2 * (s-1) +
    ...
    fem_g <- exp((grow_coef$b0_g[post_draws[p]]) +
                        (grow_coef$bsize_g[post_draws[p]]) * grow_mean_sizes$size[grow_mean_sizes$Sex==s] +
                        (grow_coef$bsizesex_g[post_draws[p]]) * grow_mean_sizes$size[grow_mean_sizes$Sex==s] * (s-1) +
                        (grow_coef$bsex_g[post_draws[p]]) * (s-1) +
                        (grow_coef$bpptgrow_g[post_draws[p]]) * grow_mean_pptgrow$pptgrow[grow_mean_pptgrow$Sex==s & grow_mean_pptgrow$pptgrow_bin==i]  +
                        ...
                        (grow_coef$bpptgrowsex_g[post_draws[p]]) * grow_mean_pptgrow$pptgrow[grow_mean_pptgrow$Sex==s & grow_mean_pptgrow$pptgrow_bin==i] * (s-1) +
                        ...
                        (grow_coef$btempgrowpptgrow_g[post_draws[p]]) * tempgrow_seq * grow_mean_pptgrow$pptgrow[grow_mean_pptgrow$Sex==s & grow_mean_pptgrow$pptgrow_bin==i] +
                        ...
                        (grow_coef$bpptgrow2_g[post_draws[p]]) * (grow_mean_pptgrow$pptgrow[grow_mean_pptgrow$Sex==s & grow_mean_pptgrow$pptgrow_bin==i])^2 +
                        ...
                        (grow_coef$bpptgrow2sex_g[post_draws[p]]) * (grow_mean_pptgrow$pptgrow[grow_mean_pptgrow$Sex==s & grow_mean_pptgrow$pptgrow_bin==i])^2 * (s-1) +
    ...
    fem_f <- invlogit((flow_coef$b0_f[post_draws[p]]) +
                        (flow_coef$bsize_f[post_draws[p]]) * flow_mean_sizes$size[flow_mean_sizes$Sex==s] +
                        (flow_coef$bsizesex_f[post_draws[p]]) * flow_mean_sizes$size[flow_mean_sizes$Sex==s] * (s-1) +
                        (flow_coef$bsex_f[post_draws[p]]) * (s-1) +
                        (flow_coef$bpptgrow_f[post_draws[p]]) * flow_mean_pptgrow$pptgrow[flow_mean_pptgrow$Sex==s & flow_mean_pptgrow$pptgrow_bin==i]  +
                        ...
                        (flow_coef$bpptgrowsex_f[post_draws[p]]) * flow_mean_pptgrow$pptgrow[flow_mean_pptgrow$Sex==s & flow_mean_sizes$size_bin==i] * (s-1) +
                        ...
                        (flow_coef$bpptgrow2_f[post_draws[p]]) * (flow_mean_pptgrow$pptgrow[flow_mean_pptgrow$Sex==s & flow_mean_sizes$size_bin==i])^2 +
                        ...
                        (flow_coef$bpptgrow2sex_f[post_draws[p]]) * (flow_mean_pptgrow$pptgrow[flow_mean_pptgrow$Sex==s & flow_mean_sizes$size_bin==i])^2 * (s-1) +
     ...
     fem_p <- exp((panic_coef$b0_p[post_draws[p]]) +
                        (panic_coef$bsize_p[post_draws[p]]) * panic_mean_sizes$size[panic_mean_sizes$Sex==s] +
                        (panic_coef$bsizesex_p[post_draws[p]]) * panic_mean_sizes$size[panic_mean_sizes$Sex==s] * (s-1) +
                        (panic_coef$bsex_p[post_draws[p]]) * (s-1) +
                        (panic_coef$bpptgrow_p[post_draws[p]]) * panic_mean_pptgrow$pptgrow[panic_mean_pptgrow$Sex==s & panic_mean_pptgrow$size_bin==i]  +
                        ...
                        (panic_coef$bpptgrowsex_p[post_draws[p]]) * panic_mean_pptgrow$pptgrow[panic_mean_pptgrow$Sex==s & panic_mean_pptgrow$size_bin==i] * (s-1) +
                        ...
                        (panic_coef$btempgrowpptgrow_p[post_draws[p]]) * tempgrow_seq * panic_mean_pptgrow$pptgrow[panic_mean_pptgrow$Sex==s & panic_mean_pptgrow$size_bin==i]   +
                        ...
                        (panic_coef$btempgrowpptgrowsex_p[post_draws[p]]) * tempgrow_seq * panic_mean_pptgrow$pptgrow[panic_mean_pptgrow$Sex==s & panic_mean_pptgrow$size_bin==i]  * (s-1) +
                        (panic_coef$bpptgrow2_p[post_draws[p]]) * (panic_mean_pptgrow$pptgrow[panic_mean_pptgrow$Sex==s & panic_mean_pptgrow$size_bin==i])^2 +
                        ...
                        (panic_coef$bpptgrow2sex_p[post_draws[p]]) * (panic_mean_pptgrow$pptgrow[panic_mean_pptgrow$Sex==s & panic_mean_pptgrow$size_bin==i])^2 * (s-1) +
   ...
   male_s <-invlogit((surv_coef$b0_s[post_draws[p]]) +
    ...
    male_f <- invlogit((flow_coef$b0_f[post_draws[p]]) +
                        ...
                        (flow_coef$btempgrow_f[post_draws[p]]) * tempgrow_seq +
                        (flow_coef$btempdorm_f[post_draws[p]]) * flow_mean_tempdorm$tempdorm[flow_mean_tempdorm$Sex==s] +
                        (flow_coef$bpptgrowsex_f[post_draws[p]]) * flow_mean_pptgrow$pptgrow[flow_mean_pptgrow$Sex==s & flow_mean_pptgrow$pptgrow_bin==i] * (s-1) +
                        (flow_coef$bpptdormsex_f[post_draws[p]]) * flow_mean_pptdorm$pptdorm[flow_mean_pptdorm$Sex==s]  * (s-1) +
                        (flow_coef$btempgrowsex_f[post_draws[p]]) * tempgrow_seq * (s-1) +
                        (flow_coef$btempdormsex_f[post_draws[p]]) * flow_mean_tempdorm$tempdorm[flow_mean_tempdorm$Sex==s] * (s-1) +
                        (flow_coef$btempdormpptdorm_f[post_draws[p]]) * flow_mean_tempdorm$tempdorm[flow_mean_tempdorm$Sex==s] * flow_mean_pptdorm$pptdorm[flow_mean_pptdorm$Sex==s]  +
...
plot(crop_tempgrow_raster,xlab="Longitude",ylab="Latitude",main="Temperature (dorm. season)",cex.main=1)
# mtext("D",side = 3, adj = 0,cex=1.25)
plot(study_area,add=T)
plot(data_gbif,add=T,pch = 23,col="black",bg="grey",cex =0.75)
plot(garden,add=T,pch = 21,col="black",cex =1.25,bg="black")
...
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Updated/Fig_geoPrlambda_miroc.pdf", Fig_geoPrlambdamiroc, width =9, height = 10)
```
...
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Updated/Fig_geoPrlambdacmc.pdf", Fig_geoPrlambdacmc, width = 9, height = 10)
```
...
Fig_geoPrlambdaacc<-ggarrange(lambda_map_Prpast, lambda_map_Prcurrent, lambda_map_Pracc45, lambda_map_Pracc85,lambda_map_Prpast_fd, lambda_map_Prcurrent_fd,lambda_map_acc45_fd,lambda_map_acc85_fd,ncol=4,nrow=2,
                                        common.legend = TRUE)
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Updated/Fig_geoPrlambdaacc_.pdf", Fig_geoPrlambdaacc, width = 9, height = 10)
```
...
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Updated/Fig_geoPrlambda_ces.pdf", Fig_geoPrlambdaces,  width = 9, height = 10)
```
...
par(mar=c(5,5,1.5,0.5),mfrow=c(2,2))
fields::image.plot(all_pptdorm_seq,all_tempdorm_seq,mtrx_dorm,col=topo.colors(100),xlab="Precipitation (dorm. season)",ylab="Temperature (dorm. season)",main="",breaks =scal_breaks ) 
...
fields::image.plot(all_pptgrow_seq,all_tempgrow_seq,mtrx_grow,col=topo.colors(100),xlab="Precipitation (grow. season)",ylab="Temperature (grow. season)",main="",breaks = scal_breaks) 
...
fields::image.plot(all_pptdorm_seq,all_tempdorm_seq,mtrx_dorm_fd,col=topo.colors(100),xlab="Precipitation (dorm. season)",ylab="Temperature (dorm. season)",main="",breaks = scal_breaks) 
...
text(x=x_miroc45_d,y=y_miroc45_d,"*",col="black",cex=2)
text(x=x_miroc85_d,y=y_miroc85_d,".",col="black",cex=4.5)
fields::image.plot(all_pptgrow_seq,all_tempgrow_seq,mtrx_grow_fd,col=topo.colors(100),xlab="Precipitation (grow. season)",ylab="Temperature (grow. season)",main="",breaks = scal_breaks) 
...
fit_garden_osr_tempgrow <- stan(
  file = '/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Analysis/stan/poar_garden_tempgrow.stan',
  data = garden_osr_dat,
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
...
fit_garden_osr_tempdorm <- stan(
  file = '/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Analysis/stan/poar_garden_temporm.stan',
  data = garden_osr_dat,
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
...
fit_garden_osr_pptgrow <- stan(
  file = '/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Analysis/stan/poar_garden_pptgrow.stan',
  data = garden_osr_dat,
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
...
fit_garden_osr_pptdorm <- stan(
  file = '/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Analysis/stan/poar_garden_pptdorm.stan',
  data = garden_osr_dat,
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
...
```{r}
traceplot(fit_garden_osr_tempgrow, pars = quote_bare(b0,b_tempgrow,b_tempgrow2))+theme_bw()
traceplot(fit_garden_osr_tempdorm, pars = quote_bare(b0,b_tempdorm,b_tempdorm2))+theme_bw()
traceplot(fit_garden_osr_pptgrow, pars = quote_bare(b0,b_pptgrow,b_pptgrow2))+theme_bw()
traceplot(fit_garden_osr_pptdorm, pars = quote_bare(b0,b_pptdorm,b_pptdorm2))+theme_bw()
```

```{r}
## pull out coefficients
coef_garden_osr_tempgrow <- rstan::extract(fit_garden_osr_tempgrow,par=c("b0","b_tempgrow","b_tempgrow2"))
coef_garden_osr_pptgrow <- rstan::extract(fit_garden_osr_pptgrow,par=c("b0","b_pptgrow","b_pptgrow2"))
coef_garden_osr_tempdorm <- rstan::extract(fit_garden_osr_tempdorm,par=c("b0","b_tempdorm","b_tempdorm2"))
coef_garden_osr_pptdorm <- rstan::extract(fit_garden_osr_pptdorm,par=c("b0","b_pptdorm","b_pptdorm2"))
```

```{r}
posterior_pptgrow <- as.array(fit_garden_osr_pptgrow)
color_scheme_set("orange")
osr_pptgrow<-mcmc_intervals(posterior_pptgrow, pars = quote_bare(b0,b_pptgrow,b_pptgrow2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_pptgrow","b_pptgrow2"),
                            labels=c("b0"="b0",
                                     "b_pptgrow"="pptgrow",
...
posterior_tempgrow <- as.array(fit_garden_osr_tempgrow)
osr_tempgrow<-mcmc_intervals(posterior_tempgrow, pars = quote_bare(b0,b_tempgrow,b_tempgrow2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_tempgrow","b_tempgrow2"),
                            labels=c("b0"="b0",
                                     "b_tempgrow"="tempgrow",
...
posterior_pptdorm <- as.array(fit_garden_osr_pptdorm)
osr_pptdorm<-mcmc_intervals(posterior_pptdorm, pars = quote_bare(b0,b_pptdorm,b_pptdorm2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_pptdorm","b_pptdorm2"),
                            labels=c("b0"="b0",
                                     "b_pptdorm"="pptdorm",
...
posterior_tempdorm <- as.array(fit_garden_osr_tempdorm)
osr_tempdorm<-mcmc_intervals(posterior_tempdorm, pars = quote_bare(b0,b_tempdorm,b_tempdorm2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_tempdorm","b_tempdorm2"),
                            labels=c("b0"="b0",
                                     "b_tempdorm"="tempdorm",
...
```{r,eval=FALSE}
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Posterior_ORS.pdf",useDingbats = F,height=6,width=5)
ggarrange(osr_pptgrow,osr_tempgrow,osr_pptdorm,osr_tempdorm + rremove("ylab"), ncol = 2, nrow = 2)
dev.off()
```
...
set.seed(13)
n_post_draws<-300
osr_post_draws <- sample.int(length(coef_garden_osr_tempgrow$b0), n_post_draws)
```
...
plot(garden_osr_poolyr$pptgrow*sd(garden_osr_poolyr_plot$pptgrow)+mean(garden_osr_poolyr_plot$pptgrow),garden_osr_poolyr$osr,ylim=c(0,1),cex.lab=1.2,cex.axis=1.2,
     xlab="Precipitation (grow.season)",ylab="Proportion female plants")
...
plot(garden_osr_poolyr$tempgrow*sd(garden_osr_poolyr_plot$tempgrow)+mean(garden_osr_poolyr_plot$tempgrow),garden_osr_poolyr$osr,ylim=c(0,1),cex.lab=1.2,cex.axis=1.2,
     xlab="Temerature (grow.season)",ylab="Proportion female plants")
abline(h=0.5,col="gray",lty=2)
mtext("B",side = 3, adj = 0,cex=1.25)
for(p in 1:n_post_draws){
...
plot(garden_osr_poolyr$pptdorm*sd(garden_osr_poolyr_plot$pptdorm)+mean(garden_osr_poolyr_plot$pptdorm),garden_osr_poolyr$osr,ylim=c(0,1),cex.lab=1.2,cex.axis=1.2,
     xlab="Precipitation (dorm.season)",ylab="Proportion female plants")
mtext("C",side = 3, adj = 0,cex=1.25)
abline(h=0.5,col="gray",lty=2)
for(p in 1:n_post_draws){
...
plot(garden_osr_poolyr$tempdorm*sd(garden_osr_poolyr_plot$tempdorm)+mean(garden_osr_poolyr_plot$tempdorm),garden_osr_poolyr$osr,ylim=c(0,1),cex.lab=1.2,cex.axis=1.2,
     xlab="Temperature (dorm.season)",ylab="Proportion female plants")
mtext("D",side = 3, adj = 0,cex=1.25)
abline(h=0.5,col="gray",lty=2)
for(p in 1:n_post_draws){
...
```{r}
traceplot(fit_garden_sr_tempgrow, pars = quote_bare(b0,b_tempgrow,b_tempgrow2))+theme_bw()
traceplot(fit_garden_sr_tempdorm, pars = quote_bare(b0,b_tempdorm,b_tempdorm2))+theme_bw()
traceplot(fit_garden_sr_pptgrow, pars = quote_bare(b0,b_pptgrow,b_pptgrow2))+theme_bw()
traceplot(fit_garden_sr_pptdorm, pars = quote_bare(b0,b_pptdorm,b_pptdorm2))+theme_bw()
```

```{r}
## pull out coefficients
coef_garden_sr_tempgrow <- rstan::extract(fit_garden_sr_tempgrow,par=c("b0","b_tempgrow","b_tempgrow2"))
coef_garden_sr_pptgrow <- rstan::extract(fit_garden_sr_pptgrow,par=c("b0","b_pptgrow","b_pptgrow2"))
coef_garden_sr_tempdorm <- rstan::extract(fit_garden_sr_tempdorm,par=c("b0","b_tempdorm","b_tempdorm2"))
coef_garden_sr_pptdorm <- rstan::extract(fit_garden_sr_pptdorm,par=c("b0","b_pptdorm","b_pptdorm2"))
```

```{r}
posterior_pptgrow_sr <- as.array(fit_garden_sr_pptgrow)
color_scheme_set("orange")
sr_pptgrow<-mcmc_intervals(posterior_pptgrow_sr, pars = quote_bare(b0,b_pptgrow,b_pptgrow2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_pptgrow","b_pptgrow2"),
                            labels=c("b0"="b0",
                                     "b_pptgrow"="pptgrow",
                                     "b_pptgrow2"=expression(pptgrow^2)))+
...
posterior_tempgrow_sr <- as.array(fit_garden_sr_tempgrow)
sr_tempgrow<-mcmc_intervals(posterior_tempgrow_sr, pars = quote_bare(b0,b_tempgrow,b_tempgrow2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_tempgrow","b_tempgrow2"),
                            labels=c("b0"="b0",
                                     "b_tempgrow"="tempgrow",
                                     "b_tempgrow2"=expression(tempgrow^2)))+
...
posterior_pptdorm_sr <- as.array(fit_garden_sr_pptdorm)
sr_pptdorm<-mcmc_intervals(posterior_pptdorm_sr, pars = quote_bare(b0,b_pptdorm,b_pptdorm2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_pptdorm","b_pptdorm2"),
                            labels=c("b0"="b0",
                                     "b_pptdorm"="pptdorm",
                                     "b_pptdorm2"=expression(pptdorm^2)))+
...
posterior_tempdorm_sr <- as.array(fit_garden_sr_tempdorm)
sr_tempdorm<-mcmc_intervals(posterior_tempdorm_sr, pars = quote_bare(b0,b_tempdorm,b_tempdorm2)) + 
  ggplot2::scale_y_discrete(limits = c("b0","b_tempdorm","b_tempdorm2"),
                            labels=c("b0"="b0",
                                     "b_tempdorm"="tempdorm",
                                     "b_tempdorm2"=expression(tempdorm^2)))+
...
```{r,eval=FALSE}
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Posterior_SR.pdf",useDingbats = F,height=6,width=5)
ggarrange(sr_pptgrow,sr_tempgrow,sr_pptdorm,sr_tempdorm + rremove("ylab"), ncol = 2, nrow = 2)
dev.off()
```
...
set.seed(13)
n_post_draws<-300
sr_post_draws <- sample.int(length(coef_garden_sr_tempgrow$b0), n_post_draws)
```
...
```{r}
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/SR_gardens_SR.pdf",useDingbats = F,width=9,height=8)
par(mar=c(5,5,1.5,0.5),mfrow=c(2,2))
...
for(p in 1:n_post_draws){
  lines(x_pptgrow*sd(garden_sr_poolyr_plot$pptgrow)+mean(garden_sr_poolyr_plot$pptgrow),
        invlogit(coef_garden_sr_pptgrow$b0[sr_post_draws[p]] +
                   coef_garden_sr_pptgrow$b_pptgrow[sr_post_draws[p]] * x_pptgrow +
                   coef_garden_sr_pptgrow$b_pptgrow2[sr_post_draws[p]] * x_pptgrow* x_pptgrow),
        col=alpha("#E6AB02",0.1))
}
...
for(p in 1:n_post_draws){
  lines(x_tempgrow*sd(garden_sr_poolyr_plot$tempgrow)+mean(garden_sr_poolyr_plot$tempgrow),
        invlogit(coef_garden_sr_tempgrow$b0[sr_post_draws[p]] +
                   coef_garden_sr_tempgrow$b_tempgrow[sr_post_draws[p]] * x_tempgrow +
                   coef_garden_sr_tempgrow$b_tempgrow2[sr_post_draws[p]] * x_tempgrow* x_tempgrow),
        col=alpha("#E6AB02",0.1))
}
...
for(p in 1:n_post_draws){
  lines(x_pptdorm*sd(garden_sr_poolyr_plot$pptdorm)+mean(garden_sr_poolyr_plot$pptdorm),
        invlogit(coef_garden_sr_pptdorm$b0[sr_post_draws[p]] +
                   coef_garden_sr_pptdorm$b_pptdorm[sr_post_draws[p]] * x_pptdorm +
                   coef_garden_sr_pptdorm$b_pptdorm2[sr_post_draws[p]] * x_pptdorm* x_pptdorm),
        col=alpha("#E6AB02",0.1))
}
...
for(p in 1:n_post_draws){
  lines(x_tempdorm*sd(garden_sr_poolyr_plot$tempdorm)+mean(garden_sr_poolyr_plot$tempdorm),
        invlogit(coef_garden_sr_tempdorm$b0[sr_post_draws[p]] +
                   coef_garden_sr_tempdorm$b_tempdorm[sr_post_draws[p]] * x_tempdorm +
                   coef_garden_sr_tempdorm$b_tempdorm2[sr_post_draws[p]] * x_tempdorm* x_tempdorm),
        col=alpha("#E6AB02",0.1))
}

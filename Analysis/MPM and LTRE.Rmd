...
library(lattice)
cls <- colorRampPalette(rev(brewer.pal(11, 'RdYlBu')))(100)
# mtrx_dorm <- matrix(lambda_dorm_mean, nrow = 10, dimnames = list(pptdorm_seq_extend,
#                tempdorm_seq_extend))
# persp(mtrx_dorm,xlab="Precipitation",ylab="Temperature",zlab=expression(paste(lambda)),
#                      theta = 30, phi = 30,col=cls)

lambda_dorm<-as.vector(lambda_dorm_mean)
datplot<-data.frame(lambda=lambda_dorm,pptdorm=climat_dorm$pptdorm_seq_extend,tempdorm=climat_dorm$tempdorm_seq_extend)
datplot<-datplot[-6,]
# mtrx_dorm <- matrix(datplot$lambda, nrow = 9, dimnames = list(datplot$pptdorm,
#                datplot$tempdorm))

trellis.par.set("axis.line",list(col=NA,lty=1,lwd=1))
wireframe(datplot$lambda ~ datplot$pptdorm + datplot$tempdorm,
          # subset = lambda_grow_mean < 5, 
          zlab = list(expression(paste(lambda)), rot=90),
    drape = TRUE, col.regions = cls, scales = list(arrows = FALSE), 
    par.settings = list(axis.line = list(col = 'transparent')),
...
filled.contour(x=tempgrow_seq_extend,y=pptgrow_seq_extend,z=mtrx_grow)
# fields::image.plot(mtrx_grow)
# filled.contour(mtrx_dorm, plot.axes = {
#   axis(1)
#   axis(2)
#   contour(mtrx_dorm, add = TRUE, lwd = 1)
#   }
# )

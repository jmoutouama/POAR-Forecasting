...
```{r}
#define quantiles of posterior samples
sex_diff_surv_mean_tempgrow <- sex_diff_grow_mean_tempgrow <- sex_diff_flow_mean_tempgrow <- sex_diff_panic_mean_tempgrow <- matrix(NA,size_bin_num,length(pptgrow_seq))
sex_diff_surv_95_tempgrow <- sex_diff_surv_75_tempgrow <- sex_diff_surv_50_tempgrow <- sex_diff_surv_25_tempgrow <- array(NA,dim=c(size_bin_num,length(pptgrow_seq),2))
sex_diff_grow_95_tempgrow <- sex_diff_grow_75_tempgrow <- sex_diff_grow_50_tempgrow <- sex_diff_grow_25_tempgrow <- array(NA,dim=c(size_bin_num,length(pptgrow_seq),2))
sex_diff_flow_95_tempgrow <- sex_diff_flow_75_tempgrow <- sex_diff_flow_50_tempgrow <- sex_diff_flow_25_tempgrow <- array(NA,dim=c(size_bin_num,length(pptgrow_seq),2))
sex_diff_panic_95_tempgrow <- sex_diff_panic_75_tempgrow <- sex_diff_panic_50_tempgrow <- sex_diff_panic_25_tempgrow <- array(NA,dim=c(size_bin_num,length(pptgrow_seq),2))
for(s in 1:size_bin_num){
  for(l in 1:length(pptgrow_seq)){
...
plot(climat_ppt$pptgrow_seq_extend,lambda_pptgrow_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3,ylim=c(0,5))
...
climat_temp<-data.frame(pptgrow_seq_extend=rep(mean(poar.clim_seasonal$pptgrow),length(pptgrow_seq_extend)),tempgrow_seq_extend=tempgrow_seq_extend,pptdorm_seq_extend=rep(mean(poar.clim_seasonal$pptdorm),length(pptgrow_seq_extend)),tempdorm_seq_extend=rep(mean(poar.clim_seasonal$tempdorm),length(pptgrow_seq_extend)))
...
ssd_pptdorm<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_ppt))
max_yrs <- 20
...
plot(climat_pptdorm$pptdorm_seq_extend,lambda_pptdorm_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation dormant season",ylab=expression(paste(lambda)),cex.lab=1.3,ylim=c(0,5))
...
plot(climat_tempdorm$tempdorm_seq_extend,lambda_tempdorm_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature dormant season",ylab=expression(paste(lambda)),cex.lab=1.3,ylim=c(0,2.7))

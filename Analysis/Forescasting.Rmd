...
#print figure
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/vital_rates1.50.5.pdf",height = 14,width = 13,useDingbats = F)
...
```{r}
data_past_present_45<-rbind(zclim_past,zclim_current,data_site_45)
```

```{r}
lambda_pp45<-lambda_pp45_2sex<-SR_pp45<-OSR_pp45<-c()
ssd_pp45<-matrix(NA,(F_params$max_size+1)*2,nrow(data_past_present_45))
max_yrs <- 30

for (i in 1:nrow(data_past_present_45)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=data_past_present_45[i,1],
                           ztempgrow=data_past_present_45[i,2],
                           zpptdorm=data_past_present_45[i,3],
                           ztempdorm=data_past_present_45[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pp45[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pp45_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_pp45[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pp45[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pp45[,i] <- lambda_run$n0
}
```

```{r}
Time<-c(rep("Past",14),rep("Present",14),rep("Future",14))
datltre45<-data.frame(data_past_present_45,lambda=lambda_pp45_2sex,Time=Time)
...
```
```{r}
ou45<-tuneRF(y=datltre45$lambda, x=as.data.frame(datltre45[,-c(5,6)]),stepFactor = 1,improve = 0.02)
plot(ou45)
modrf45<-randomForest::randomForest(y=datltre45$lambda, x=as.data.frame(datltre45[,-c(5,6)]),ntree=500,importance=TRUE,mtry=1)
plot(modrf45$rsq,type="l")
randomForest::varImpPlot(modrf45,type = 1,scale = FALSE,main="")
```

```{r}
# Get importance values as a data frame
library(randomForest)
imp45 = as.data.frame(importance(modrf45))
row.names(imp45)<-NULL
imp45$vars<-c("pptgrow","tempgrow","pptdorm","tempdorm")
imp45<-imp45[,-2]
colnames(imp45)<-c("Importance","vars")
p<-ggplot(data=imp45, aes(x=vars, y=Importance)) +
...
```{r}
all_tempgrow<-c(zclim_current$ztempgrow,zclim_past$ztempgrow,zclim_miroc45$ztempgrow,zclim_miroc85$ztempgrow)
all_tempgrow_seq <- seq(min(all_tempgrow),max(all_tempgrow),length.out=20)
...
all_pptdorm_seq <- seq(min(all_pptdorm),max(all_pptdorm),length.out=20)

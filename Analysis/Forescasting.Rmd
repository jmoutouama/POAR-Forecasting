...
This is the code for the paper 'Forecasting range shifts of a dioecious plant species under climate change', which  was  submitted in *Ecological letters* in 2024.
...
hist(clim_current$pptgrow,xlab="Growing",xlim=c(50,1000),ylim=c(0,5),main="")
...
hist(clim_current$tempgrow,xlab="Growing",xlim=c(5,30),main="")
...
hist(clim_current$pptdorm,xlab="Dormant",xlim=c(100,500),main="")
...
hist(clim_current$tempdorm,xlab="Dormant",xlim=c(20,35),ylim=c(0,6),main="")
...
mcmc_intervals(posterior_season, pars = quote_bare(b0_s,bsizesex_s,btempgrowpptgrow_s,bsex_s,
                                                        bsize_s,bpptgrow_s,btempgrow_s,
                                                        bpptgrowsex_s,btempgrowsex_s,
                                                        btempgrowpptgrowsex_s,bpptgrow2_s,
                                                        btempgrow2_s,bpptgrow2sex_s,
...
```{r}
mcmc_intervals(posterior_season, pars = quote_bare(b0_g,btempgrowpptgrow_g,bsizesex_g,bsex_g,
                                                        bsize_g,bpptgrow_g,btempgrow_g,bpptgrowsex_g,
                                                        btempgrowsex_g,
                                                        btempgrowpptgrowsex_g,bpptgrow2_g,btempgrow2_g,
...
```{r}
mcmc_intervals(posterior_season, pars = quote_bare(b0_f,bsizesex_f,bsex_f, bsize_f,bpptgrow_f,btempgrow_f,
                                                        bpptgrowsex_f,btempgrowsex_f,
                                                        btempgrowpptgrow_f,
                                                        btempgrowpptgrowsex_f,bpptgrow2_f,btempgrow2_f,
...
```{r}
mcmc_intervals(posterior_season, pars = quote_bare(b0_p,bsex_p,bsex_p,bsizesex_p, bsize_p,bpptgrow_p,
                                                         btempgrow_p,bpptgrowsex_p,btempgrowsex_p,
                                                         btempgrowpptgrow_p,
                                                         btempgrowpptgrowsex_p,bpptgrow2_p,btempgrow2_p,
...
#print figure
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/vital_rates_22_10_23.pdf",height = 14,width = 13,useDingbats = F)
...
with(poar_surv_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptgrow[size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),mean_surv[size_bin==i],type="n",xaxt="n",ylim=c(0,1),
         xlab=" ",ylab=" ");box()
    if(i==1){mtext("Pr(survival)",side=2,line=3,cex=1.3)}
    title(LETTERS[i],adj=0)
    for(s in 1:2){
      points(pptgrow[Sex==s & size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),mean_surv[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),invlogit(mean(surv_coef$b0_s) + 
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,sex_diff_surv_mean_pptgrow[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),rev(pptgrow_seq *sd(clim_current$pptgrow) + mean(clim_current$pptgrow))),
    ...
    polygon(x=c(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,rev(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptgrow_seq *sd(clim_current$pptgrow) + mean(clim_current$pptgrow),sex_diff_surv_mean_pptgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_surv_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempgrow[size_bin==i]*sd(clim_current$tempgrow)+ mean(clim_current$tempgrow),mean_surv[size_bin==i],type="n",ylim=c(0,1),
         xlab=" ",ylab=" ",xaxt="n");box()
    # if(i==1){mtext("",side=2,line=3,cex=1.3)}
    title(LETTERS[i+1],adj=0)
    for(s in 1:2){
      points(tempgrow[Sex==s & size_bin==i]*sd(clim_current$tempgrow)+mean(clim_current$tempgrow),mean_surv[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow),invlogit(mean(surv_coef$b0_s) +
                        mean(surv_coef$bsize_s) * surv_mean_sizes$size[surv_mean_sizes$Sex==s & surv_mean_sizes$size_bin==i] +
                        mean(surv_coef$bsizesex_s) * surv_mean_sizes$size[surv_mean_sizes$Sex==s & surv_mean_sizes$size_bin==i] * (s-1) +
                        mean(surv_coef$bsex_s) * (s-1) +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempgrow_seq *sd(clim_current$tempgrow) + mean(clim_current$tempgrow),sex_diff_surv_mean_tempgrow[i,],type="n",lwd=4,yaxt="n",
         xlab=" ",ylab=" ",xaxt="n",ylim=c(min(sex_diff_surv_95_tempgrow[i,,1]),max(sex_diff_surv_95_tempgrow[i,,2])))
    # if(i==1){mtext(expression(paste(Delta," (F-M)")),side=2,line=2,cex=1.3)}
    polygon(x=c(tempgrow_seq*sd(clim_current$tempgrow)+ mean(clim_current$tempgrow),rev(tempgrow_seq*sd(clim_current$tempgrow)+ mean(clim_current$tempgrow) )),
    ...
    polygon(x=c(tempgrow_seq*sd(clim_current$tempgrow)+ mean(clim_current$tempgrow) ,rev(tempgrow_seq*sd(clim_current$tempgrow)+ mean(clim_current$tempgrow))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempgrow_seq*sd(clim_current$tempgrow)+ mean(clim_current$tempgrow) ,sex_diff_surv_mean_tempgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_surv_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptdorm[size_bin==i]*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),mean_surv[size_bin==i],type="n",ylim=c(0,1),
         xlab=" ",ylab=" ",xaxt="n");box()
    # if(i==1){mtext("Pr(survival)",side=2,line=3,cex=1.3)}
    title(LETTERS[i+2],adj=0)
    for(s in 1:2){
      points(pptdorm[Sex==s & size_bin==i]*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),mean_surv[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),invlogit(mean(surv_coef$b0_s) + 
                         mean(surv_coef$bsize_s) * surv_mean_sizes$size[surv_mean_sizes$Sex==s & surv_mean_sizes$size_bin==i]+
                         mean(surv_coef$bsizesex_s) * surv_mean_sizes$size[surv_mean_sizes$Sex==s & surv_mean_sizes$size_bin==i] * (s-1) +
                         mean(surv_coef$bsex_s) * (s-1) +
                         mean(surv_coef$bpptgrow_s) * surv_mean_pptgrow$pptgrow[surv_mean_pptgrow$Sex==s] +
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptdorm_seq *sd(clim_current$pptdorm) + mean(clim_current$pptdorm),sex_diff_surv_mean_pptdorm[i,],type="n",lwd=4,yaxt="n",
         xlab=" ",ylab=" ",xaxt="n",ylim=c(min(sex_diff_surv_95_pptdorm[i,,1]),max(sex_diff_surv_95_pptdorm[i,,2])))
    # if(i==1){mtext(expression(paste(Delta," (F-M)")),side=2,line=2,cex=1.3)}
    polygon(x=c(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),rev(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) )),
    ...
    polygon(x=c(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) ,rev(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) ,sex_diff_surv_mean_pptdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_surv_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempdorm[size_bin==i]*sd(clim_current$tempdorm) + mean(clim_current$tempdorm),mean_surv[size_bin==i],type="n",ylim=c(0,1),
         xlab=" ",ylab=" ",xaxt="n");box()
    # if(i==1){mtext("Pr(survival)",side=2,line=3,cex=1.3)}
    title(LETTERS[i+3],adj=0)
    for(s in 1:2){
      points(tempdorm[Sex==s & size_bin==i]*sd(clim_current$tempdorm) + mean(clim_current$tempdorm),mean_surv[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm),invlogit(mean(surv_coef$b0_s) + 
                         mean(surv_coef$bsize_s) * surv_mean_sizes$size[surv_mean_sizes$Sex==s & surv_mean_sizes$size_bin==i] +
                         mean(surv_coef$bsizesex_s) * surv_mean_sizes$size[surv_mean_sizes$Sex==s & surv_mean_sizes$size_bin==i] * (s-1) +
                         mean(surv_coef$bsex_s) * (s-1) +
                         mean(surv_coef$bpptgrow_s) * surv_mean_pptgrow$pptgrow[surv_mean_pptgrow$Sex==s] +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempdorm_seq *sd(clim_current$tempdorm) + mean(clim_current$tempdorm),sex_diff_surv_mean_tempdorm[i,],type="n",lwd=4,yaxt="n",
         xlab=" ",ylab=" ",xaxt="n",ylim=c(min(sex_diff_surv_95_tempdorm[i,,1]),max(sex_diff_surv_95_tempdorm[i,,2])))
    # if(i==1){mtext(expression(paste(Delta," (F-M)")),side=2,line=2,cex=1.3)}
    polygon(x=c(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm),rev(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) )),
    ...
    polygon(x=c(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,rev(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,sex_diff_surv_mean_tempdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_grow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptgrow[size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),mean_grow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()    
    if(i==1){mtext("#tillers",side=2,line=3,cex=1.3)}
    title(LETTERS[i+4],adj=0)    
    for(s in 1:2){
      points(pptgrow[Sex==s & size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,mean_grow[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptgrow_seq *sd(clim_current$pptgrow) + mean(clim_current$pptgrow),sex_diff_grow_mean_pptgrow[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,rev(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,sex_diff_grow_mean_pptgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_grow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempgrow[size_bin==i]*sd(clim_current$tempgrow) + mean(clim_current$tempgrow),mean_grow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()    
    # if(i==1){mtext("#tillers",side=2,line=3,cex=1.3)}
    title(LETTERS[i+5],adj=0)    
    for(s in 1:2){
      points(tempgrow[Sex==s & size_bin==i]*sd(clim_current$tempgrow) + mean(clim_current$tempgrow),mean_grow[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow) ,
            exp(mean(grow_coef$b0_g) +
                        mean(grow_coef$bsize_g) * grow_mean_sizes$size[grow_mean_sizes$Sex==s & grow_mean_sizes$size_bin==i] +
                        mean(grow_coef$bsizesex_g) * grow_mean_sizes$size[grow_mean_sizes$Sex==s & grow_mean_sizes$size_bin==i] * (s-1) +
                        mean(grow_coef$bsex_g) * (s-1) +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow) ,sex_diff_grow_mean_tempgrow[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(tempgrow_seq *sd(clim_current$tempgrow) + mean(clim_current$tempgrow),rev(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow) ,sex_diff_grow_mean_tempgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_grow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptdorm[size_bin==i]*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),mean_grow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()    
    # if(i==1){mtext("#tillers",side=2,line=3,cex=1.3)}
    title(LETTERS[i+6],adj=0)    
    for(s in 1:2){
      points(pptdorm[Sex==s & size_bin==i]*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),mean_grow[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) ,
            exp(mean(grow_coef$b0_g) + 
                         mean(grow_coef$bsize_g) * grow_mean_sizes$size[grow_mean_sizes$Sex==s & grow_mean_sizes$size_bin==i]+
                         mean(grow_coef$bsizesex_g) * grow_mean_sizes$size[grow_mean_sizes$Sex==s & grow_mean_sizes$size_bin==i] * (s-1) +
                         mean(grow_coef$bsex_g) * (s-1) +
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptdorm_seq *sd(clim_current$pptdorm) + mean(clim_current$pptdorm),sex_diff_grow_mean_pptdorm[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) ,rev(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptdorm_seq *sd(clim_current$pptdorm) + mean(clim_current$pptdorm),sex_diff_grow_mean_pptdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_grow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempdorm[size_bin==i]*sd(clim_current$tempdorm) + mean(clim_current$tempdorm),mean_grow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()    
    # if(i==1){mtext("#tillers",side=2,line=3,cex=1.3)}
    title(LETTERS[i+7],adj=0)    
    for(s in 1:2){
      points(tempdorm[Sex==s & size_bin==i]*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,mean_grow[Sex==s & size_bin==i]*sd(clim_current$tempdorm) + mean(clim_current$tempdorm),
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(tempdorm_seq *sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,exp(mean(grow_coef$b0_g) + 
                         mean(grow_coef$bsize_g) * grow_mean_sizes$size[grow_mean_sizes$Sex==s & grow_mean_sizes$size_bin==i] +
                         mean(grow_coef$bsizesex_g) * grow_mean_sizes$size[grow_mean_sizes$Sex==s & grow_mean_sizes$size_bin==i] * (s-1) +
                         mean(grow_coef$bsex_g) * (s-1) +
                         mean(grow_coef$bpptgrow_g) * grow_mean_pptgrow$pptgrow[grow_mean_pptgrow$Sex==s] +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempdorm_seq *sd(clim_current$tempdorm) + mean(clim_current$tempdorm),sex_diff_grow_mean_tempdorm[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,rev(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempdorm_seq *sd(clim_current$tempdorm) + mean(clim_current$tempdorm),sex_diff_grow_mean_tempdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_flow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptgrow[size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),mean_flow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()
    if(i==1){mtext("Pr(flowering)",side=2,line=3,cex=1.3)}
    title(LETTERS[i+8],adj=0)    
    for(s in 1:2){
      points(pptgrow[Sex==s & size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),mean_flow[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(pptgrow_seq *sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,invlogit(mean(flow_coef$b0_f) + 
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,sex_diff_flow_mean_pptgrow[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,rev(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptgrow_seq *sd(clim_current$pptgrow) + mean(clim_current$pptgrow),sex_diff_flow_mean_pptgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_flow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempgrow[size_bin==i]*sd(clim_current$tempgrow) + mean(clim_current$tempgrow) ,mean_flow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()
    # if(i==1){mtext("Pr(flowering)",side=2,line=3,cex=1.3)}
    title(LETTERS[i+9],adj=0)    
    for(s in 1:2){
      points(tempgrow[Sex==s & size_bin==i]*sd(clim_current$tempgrow) + mean(clim_current$tempgrow),mean_flow[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(tempgrow_seq *sd(clim_current$tempgrow) + mean(clim_current$tempgrow),invlogit(mean(flow_coef$b0_f) +
                        mean(flow_coef$bsize_f) * flow_mean_sizes$size[flow_mean_sizes$Sex==s & flow_mean_sizes$size_bin==i] +
                        mean(flow_coef$bsizesex_f) * flow_mean_sizes$size[flow_mean_sizes$Sex==s & flow_mean_sizes$size_bin==i] * (s-1) +
                        mean(flow_coef$bsex_f) * (s-1) +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempgrow_seq *sd(clim_current$tempgrow) + mean(clim_current$tempgrow),sex_diff_flow_mean_tempgrow[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow) ,rev(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow) )),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow) ,sex_diff_flow_mean_tempgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_flow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptdorm[size_bin==i]*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) ,mean_flow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()
    # if(i==1){mtext("Pr(flowering)",side=2,line=3,cex=1.3)}
    title(LETTERS[i+10],adj=0)    
    for(s in 1:2){
      points(pptdorm[Sex==s & size_bin==i]*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),mean_flow[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm), invlogit(mean(flow_coef$b0_f) + 
                         mean(flow_coef$bsize_f) * flow_mean_sizes$size[flow_mean_sizes$Sex==s & flow_mean_sizes$size_bin==i]+
                         mean(flow_coef$bsizesex_f) * flow_mean_sizes$size[flow_mean_sizes$Sex==s & flow_mean_sizes$size_bin==i] * (s-1) +
                         mean(flow_coef$bsex_f) * (s-1) +
                         mean(flow_coef$bpptgrow_f) * flow_mean_pptgrow$pptgrow[flow_mean_pptgrow$Sex==s] +
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) ,sex_diff_flow_mean_pptdorm[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm) ,rev(pptdorm_seq*sd(clim_current$pptdorm) + mean(clim_current$pptdorm))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptdorm_seq *sd(clim_current$pptdorm) + mean(clim_current$pptdorm),sex_diff_flow_mean_pptdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_flow_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempdorm[size_bin==i]*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,mean_flow[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n");box()
    # if(i==1){mtext("Pr(flowering)",side=2,line=3,cex=1.3)}
    title(LETTERS[i+11],adj=0)    
    for(s in 1:2){
      points(tempdorm[Sex==s & size_bin==i]*sd(clim_current$tempdorm) + mean(clim_current$tempdorm),mean_flow[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=5*(bin_n/max(bin_n)),lwd=2)
      lines(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm), invlogit(mean(flow_coef$b0_f[post_draws[p]]) + 
                         mean(flow_coef$bsize_f) * flow_mean_sizes$size[flow_mean_sizes$Sex==s & flow_mean_sizes$size_bin==i] +
                         mean(flow_coef$bsizesex_f) * flow_mean_sizes$size[flow_mean_sizes$Sex==s & flow_mean_sizes$size_bin==i] * (s-1) +
                         mean(flow_coef$bsex_f) * (s-1) +
                         mean(flow_coef$bpptgrow_f) * flow_mean_pptgrow$pptgrow[flow_mean_pptgrow$Sex==s] +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,sex_diff_flow_mean_tempdorm[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,rev(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm))),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempdorm_seq*sd(clim_current$tempdorm) + mean(clim_current$tempdorm) ,sex_diff_flow_mean_tempdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_panic_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptgrow[size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,mean_panic[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n",ylim=c(0,max(mean_panic[size_bin==i])));box()    
    if(i==1){mtext("#panicles",side=2,line=3,cex=1.3)}
    title(LETTERS[i+12],adj=0)    
    for(s in 1:2){
      points(pptgrow[Sex==s & size_bin==i]*sd(clim_current$pptgrow) + mean(clim_current$pptgrow),mean_panic[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=7*(bin_n/max(bin_n)),lwd=2)
      lines(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow), exp(mean(panic_coef$b0_p) + 
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,sex_diff_panic_mean_pptgrow[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) ,rev(pptgrow_seq*sd(clim_current$pptgrow) + mean(clim_current$pptgrow) )),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptgrow_seq *sd(clim_current$pptgrow) + mean(clim_current$pptgrow),sex_diff_panic_mean_pptgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_panic_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempgrow[size_bin==i]* sd(clim_current$tempgrow) + mean(clim_current$tempgrow),mean_panic[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n",ylim=c(0,max(mean_panic[size_bin==i])));box()    
    # if(i==1){mtext("#panicles",side=2,line=3,cex=1.3)}
    title(LETTERS[i+13],adj=0)    
    for(s in 1:2){
      points(tempgrow[Sex==s & size_bin==i]*sd(clim_current$tempgrow) + mean(clim_current$tempgrow),mean_panic[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=7*(bin_n/max(bin_n)),lwd=2)
      lines(tempgrow_seq*sd(clim_current$tempgrow) + mean(clim_current$tempgrow), exp(mean(panic_coef$b0_p) +
                        mean(panic_coef$bsize_p) * panic_mean_sizes$size[panic_mean_sizes$Sex==s & panic_mean_sizes$size_bin==i] +
                        mean(panic_coef$bsizesex_p) * panic_mean_sizes$size[panic_mean_sizes$Sex==s & panic_mean_sizes$size_bin==i] * (s-1) +
                        mean(panic_coef$bsex_p) * (s-1) +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempgrow_seq*sd(clim_current$tempgrow) +mean(clim_current$tempgrow) ,sex_diff_panic_mean_tempgrow[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(tempgrow_seq *sd(clim_current$tempgrow) +mean(clim_current$tempgrow),rev(tempgrow_seq* sd(clim_current$tempgrow) + mean(clim_current$tempgrow) )),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempgrow_seq *sd(clim_current$tempgrow) +mean(clim_current$tempgrow),sex_diff_panic_mean_tempgrow[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_panic_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(pptdorm[size_bin==i]*sd(clim_current$pptdorm) +mean(clim_current$pptdorm),mean_panic[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n",ylim=c(0,max(mean_panic[size_bin==i])));box()    
    # if(i==1){mtext("#panicles",side=2,line=3,cex=1.3)}
    title(LETTERS[i+14],adj=0)    
    for(s in 1:2){
      points(pptdorm[Sex==s & size_bin==i]*sd(clim_current$pptdorm) + mean(clim_current$pptdorm),mean_panic[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=7*(bin_n/max(bin_n)),lwd=2)
      lines(pptdorm_seq*sd(clim_current$pptdorm) +mean(clim_current$pptdorm), exp(mean(panic_coef$b0_p) + 
                         mean(panic_coef$bsize_p) * panic_mean_sizes$size[panic_mean_sizes$Sex==s & panic_mean_sizes$size_bin==i]+
                         mean(panic_coef$bsizesex_p) * panic_mean_sizes$size[panic_mean_sizes$Sex==s & panic_mean_sizes$size_bin==i] * (s-1) +
                         mean(panic_coef$bsex_p) * (s-1) +
    ...
    par(mar=c(2,4,0.5,0))
    plot(pptdorm_seq *sd(clim_current$pptdorm) +mean(clim_current$pptdorm),sex_diff_panic_mean_pptdorm[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(pptdorm_seq*sd(clim_current$pptdorm) +mean(clim_current$pptdorm) ,rev(pptdorm_seq*sd(clim_current$pptdorm) +mean(clim_current$pptdorm) )),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(pptdorm_seq*sd(clim_current$pptdorm) +mean(clim_current$pptdorm) ,sex_diff_panic_mean_pptdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
with(poar_panic_binned,{
  for(i in 1:size_bin_num){
    par(mar=c(0,4,1,0))
    plot(tempdorm[size_bin==i]*sd(clim_current$tempdorm)+ mean(clim_current$tempdorm) ,mean_panic[size_bin==i],type="n",
         xlab=" ",ylab=" ",xaxt="n",ylim=c(0,max(mean_panic[size_bin==i])));box()    
    # if(i==1){mtext("#panicles",side=2,line=3,cex=1.3)}
    title(LETTERS[i+15],adj=0)    
    for(s in 1:2){
      points(tempdorm[Sex==s & size_bin==i]*sd(clim_current$tempdorm)+ mean(clim_current$tempdorm),mean_panic[Sex==s & size_bin==i],
             bg=sex_cols[s],pch=21,cex=7*(bin_n/max(bin_n)),lwd=2)
      lines(tempdorm_seq*sd(clim_current$tempdorm)+ mean(clim_current$tempdorm), exp(mean(panic_coef$b0_p) + 
                         mean(panic_coef$bsize_p) * panic_mean_sizes$size[panic_mean_sizes$Sex==s & panic_mean_sizes$size_bin==i] +
                         mean(panic_coef$bsizesex_p) * panic_mean_sizes$size[panic_mean_sizes$Sex==s & panic_mean_sizes$size_bin==i] * (s-1) +
                         mean(panic_coef$bsex_p) * (s-1) +
                         mean(panic_coef$bpptgrow_p) * panic_mean_pptgrow$pptgrow[panic_mean_pptgrow$Sex==s] +
    ...
    par(mar=c(2,4,0.5,0))
    plot(tempdorm_seq*sd(clim_current$tempdorm)+ mean(clim_current$tempdorm) ,sex_diff_panic_mean_tempdorm[i,],type="n",lwd=4,yaxt="n",
    ...
    polygon(x=c(tempdorm_seq*sd(clim_current$tempdorm)+ mean(clim_current$tempdorm) ,rev(tempdorm_seq*sd(clim_current$tempdorm)+ mean(clim_current$tempdorm) )),
            ...
            col=alpha(diff_col,diff_alpha),border=NA)
    lines(tempdorm_seq *sd(clim_current$tempdorm)+ mean(clim_current$tempdorm),sex_diff_panic_mean_tempdorm[i,],lwd=2,col="black")
    abline(h=0,lty=2)
    mtext("0",side=2,at=0)
  }
})
...
pptgrow_seq_extend <- seq(min(poar_2015_2016$zpptgrow),max(poar_2015_2016$zpptgrow),length.out=20)
climat_ppt<-data.frame(pptgrow_seq_extend=pptgrow_seq_extend,tempgrow_seq_extend=rep(mean(poar_2015_2016$ztempgrow),length(pptgrow_seq_extend)),pptdorm_seq_extend=rep(mean(poar_2015_2016$zpptdorm),length(pptgrow_seq_extend)),tempdorm_seq_extend=rep(mean(poar_2015_2016$ztempdorm),length(pptgrow_seq_extend)))
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_ppt$pptgrow_seq_extend,lambda_pptgrow_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt$pptgrow_seq_extend,lambda_pptgrow_mean_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```

### Temperature of the growing season
```{r}
## note that length of this vector has a big effect on how fast/slow the following code runs
tempgrow_seq_extend <- seq(min(poar_2015_2016$ztempgrow),max(poar_2015_2016$ztempgrow),length.out=20)
climat_temp<-data.frame(pptgrow_seq_extend=rep(mean(poar_2015_2016$zpptgrow),length(pptgrow_seq_extend)),tempgrow_seq_extend=tempgrow_seq_extend,pptdorm_seq_extend=rep(mean(poar_2015_2016$zpptdorm),length(tempgrow_seq_extend)),tempdorm_seq_extend=rep(mean(poar_2015_2016$ztempdorm),length(tempgrow_seq_extend)))
...
for (i in 1:nrow(climat_temp)) {
  ...
  mat <- megamatrix_delay( F_params=F_params,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_temp[i,1],
                           ztempgrow=climat_temp[i,2],
                           zpptdorm=climat_temp[i,3],
                           ztempdorm=climat_temp[i,4],
                           rfx=rfx_fun())$MEGAmat
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_temp$tempgrow_seq_extend,lambda_tempgrow_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_temp$tempgrow_seq_extend,lambda_tempgrow_mean_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
pptdorm_seq_extend <- seq(min(poar_2015_2016$zpptdorm),max(poar_2015_2016$zpptdorm),length.out=20)
climat_pptdorm<-data.frame(pptgrow_seq_extend=rep(mean(poar_2015_2016$zpptgrow),length(pptdorm_seq_extend)),tempgrow_seq_extend=rep(mean(poar_2015_2016$ztempgrow),length(pptdorm_seq_extend)),pptdorm_seq_extend=pptdorm_seq_extend,tempdorm_seq_extend=rep(mean(poar_2015_2016$ztempdorm),length(pptdorm_seq_extend)))
...
for (i in 1:nrow(climat_pptdorm)) {
  ...
  mat <- megamatrix_delay( F_params=F_params,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_pptdorm[i,1],
                           ztempgrow=climat_pptdorm[i,2],
                           zpptdorm=climat_pptdorm[i,3],
                           ztempdorm=climat_pptdorm[i,4],
                           rfx=rfx_fun())$MEGAmat
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_pptdorm$pptdorm_seq_extend,lambda_pptdorm_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation of the dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_pptdorm$pptdorm_seq_extend,lambda_pptdorm_mean_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
tempdorm_seq_extend <- seq(min(poar_2015_2016$ztempdorm),max(poar_2015_2016$ztempdorm),length.out=20)
climat_tempdorm<-data.frame(pptgrow_seq_extend=rep(mean(poar_2015_2016$zpptgrow),length(tempdorm_seq_extend)),tempgrow_seq_extend=rep(mean(poar_2015_2016$ztempgrow),length(tempdorm_seq_extend)),pptdorm_seq_extend=rep(mean(poar_2015_2016$zpptdorm),length(tempdorm_seq_extend)),tempdorm_seq_extend=tempdorm_seq_extend)
lambda_tempdorm_mean<-lambda_tempdorm_mean_2sex<-SR_tempdorm_mean<-OSR_tempdorm_mean<-c()
ssd_tempdorm<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_temp))
max_yrs <- 20
...
for (i in 1:nrow(climat_tempdorm)) {
  ...
  mat <- megamatrix_delay( F_params=F_params,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_tempdorm[i,1],
                           ztempgrow=climat_tempdorm[i,2],
                           zpptdorm=climat_tempdorm[i,3],
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_tempdorm$tempdorm_seq_extend,lambda_tempdorm_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature of the dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_tempdorm$tempdorm_seq_extend,lambda_tempdorm_mean_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```
...
```{r}
## extend the climatic  to bounds and beyond of observed distribution
pptgrow_seq_extend <- seq(min(zclim_current$zpptgrow),max(zclim_current$zpptgrow),length.out=20)
climat_ppt<-data.frame(pptgrow_seq_extend=pptgrow_seq_extend,tempgrow_seq_extend=rep(mean(zclim_current$ztempgrow),length(pptgrow_seq_extend)),pptdorm_seq_extend=rep(mean(zclim_current$zpptdorm),length(pptgrow_seq_extend)),tempdorm_seq_extend=rep(mean(zclim_current$ztempdorm),length(pptgrow_seq_extend)))
lambda_pptgrow_mean<-lambda_pptgrow_mean_2sex<-SR_pptgrow_mean<-OSR_pptgrow_mean<-c()
ssd_pptgrow<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_ppt))
max_yrs <- 20
...
```{r}
poar_2015_2016_plot<-poar_2015_2016[,c(9,8,11,10)]
zclim_current_combo <- data.frame(scale(rbind(poar_2015_2016_plot,clim_current)))
colnames(zclim_current_combo)<-c("zpptgrow","ztempgrow","zpptdorm","ztempdorm")
```

```{r}
## extend the climatic  to bounds and beyond of observed distribution
pptgrow_seq <- seq(min(zclim_current_combo$zpptgrow),max(zclim_current_combo$zpptgrow),length.out=20)
climat_ppt_combo<-data.frame(pptgrow_seq=pptgrow_seq,tempgrow_seq=rep(mean(zclim_current_combo$ztempgrow),length(pptgrow_seq)),pptdorm_seq=rep(mean(zclim_current_combo$zpptdorm),length(pptgrow_seq)),tempdorm_seq=rep(mean(zclim_current_combo$ztempdorm),length(pptgrow_seq)))

lambda_pptgrow_mean<-lambda_pptgrow_mean_2sex<-SR_pptgrow_mean<-OSR_pptgrow_mean<-c()
ssd_pptgrow<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_ppt_combo))
max_yrs <- 20

for (i in 1:nrow(climat_ppt_combo)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_ppt_combo[i,1],
                           ztempgrow=climat_ppt_combo[i,2],
                           zpptdorm=climat_ppt_combo[i,3],
                           ztempdorm=climat_ppt_combo[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pptgrow_mean[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_ppt_combo$pptgrow,lambda_pptgrow_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt_combo$pptgrow,lambda_pptgrow_mean_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```
...
```{r}
## note that length of this vector has a big effect on how fast/slow the following code runs
tempgrow_seq_extend <- seq(min(zclim_current$ztempgrow),max(zclim_current$ztempgrow),length.out=20)
climat_temp<-data.frame(pptgrow_seq_extend=rep(mean(zclim_current$zpptgrow),length(pptgrow_seq_extend)),tempgrow_seq_extend=tempgrow_seq_extend,pptdorm_seq_extend=rep(mean(zclim_current$zpptdorm),length(tempgrow_seq_extend)),tempdorm_seq_extend=rep(mean(zclim_current$ztempdorm),length(tempgrow_seq_extend)))
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
pptdorm_seq_extend <- seq(min(zclim_current$zpptdorm),max(zclim_current$zpptdorm),length.out=20)
climat_pptdorm<-data.frame(pptgrow_seq_extend=rep(mean(zclim_current$zpptgrow),length(pptdorm_seq_extend)),tempgrow_seq_extend=rep(mean(zclim_current$ztempgrow),length(pptdorm_seq_extend)),pptdorm_seq_extend=pptdorm_seq_extend,tempdorm_seq_extend=rep(mean(zclim_current$ztempdorm),length(pptdorm_seq_extend)))
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
tempdorm_seq_extend <- seq(min(zclim_current$ztempdorm),max(zclim_current$ztempdorm),length.out=20)
climat_tempdorm<-data.frame(pptgrow_seq_extend=rep(mean(zclim_current$zpptgrow),length(tempdorm_seq_extend)),tempgrow_seq_extend=rep(mean(zclim_current$ztempgrow),length(tempdorm_seq_extend)),pptdorm_seq_extend=rep(mean(zclim_current$zpptdorm),length(tempdorm_seq_extend)),tempdorm_seq_extend=tempdorm_seq_extend)
lambda_tempdorm_mean<-lambda_tempdorm_mean_2sex<-SR_tempdorm_mean<-OSR_tempdorm_mean<-c()
ssd_tempdorm<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_temp))
max_yrs <- 20
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
tempdorm_seq <- seq(min(zclim_current_combo$ztempdorm),max(zclim_current_combo$ztempdorm),length.out=20)
climat_tempdorm_combo<-data.frame(pptgrow_seq=rep(mean(zclim_current_combo$zpptgrow),length(tempdorm_seq)),tempgrow_seq=rep(mean(zclim_current_combo$ztempgrow),length(tempdorm_seq)),pptdorm_seq=rep(mean(zclim_current_combo$zpptdorm),length(tempdorm_seq)),tempdorm_seq=tempdorm_seq)
lambda_tempdorm_mean<-lambda_tempdorm_mean_2sex<-SR_tempdorm_mean<-OSR_tempdorm_mean<-c()
ssd_tempdorm<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_tempdorm_combo))
max_yrs <- 20

for (i in 1:nrow(climat_tempdorm_combo)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_tempdorm_combo[i,1],
                           ztempgrow=climat_tempdorm_combo[i,2],
                           zpptdorm=climat_tempdorm_combo[i,3],
                           ztempdorm=climat_tempdorm_combo[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_tempdorm_mean[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_tempdorm_combo$tempdorm_seq,lambda_tempdorm_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature of the dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_tempdorm_combo$tempdorm_seq,lambda_tempdorm_mean_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```

# Future and historical data vs lambda
...
```{r}
pptgrow_seq_h <- seq(min(zclim_past$zpptgrow),max(zclim_past$zpptgrow),length.out=20)
climat_ppt_h<-data.frame(pptgrow_seq_h=pptgrow_seq_h,tempgrow_seq_h=rep(mean(zclim_past$ztempgrow),length(pptgrow_seq_h)),pptdorm_seq_h=rep(mean(zclim_past$zpptdorm),length(pptgrow_seq_h)),tempdorm_seq_extend=rep(mean(zclim_past$ztempdorm),length(pptgrow_seq_h)))
lambda_pptgrow_mean_h<-lambda_pptgrow_mean_2sex_h<-SR_pptgrow_mean_h<-OSR_pptgrow_mean_h<-c()
ssd_pptgrow_h<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_ppt_h))
max_yrs <- 20

for (i in 1:nrow(climat_ppt_h)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_ppt_h[i,1],
                           ztempgrow=climat_ppt_h[i,2],
                           zpptdorm=climat_ppt_h[i,3],
                           ztempdorm=climat_ppt_h[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pptgrow_mean_h[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pptgrow_mean_2sex_h[i] <- lambda_run$lambdatracker[max_yrs]
  SR_pptgrow_mean_h[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pptgrow_mean_h[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pptgrow_h[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_ppt_h$pptgrow_seq_h,lambda_pptgrow_mean_h,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt_h$pptgrow_seq_h,lambda_pptgrow_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
tempgrow_seq_h <- seq(min(zclim_past$ztempgrow),max(zclim_past$ztempgrow),length.out=20)

climat_temp_h<-data.frame(pptgrow_seq_h=rep(mean(zclim_past$zpptgrow),length(tempgrow_seq_h)),tempgrow_seq_h=tempgrow_seq_h,pptdorm_seq_h=rep(mean(zclim_past$zpptdorm),length(tempgrow_seq_h)),tempdorm_seq_extend=rep(mean(zclim_past$ztempdorm),length(tempgrow_seq_h)))

lambda_tempgrow_mean_h<-lambda_tempgrow_mean_2sex_h<-SR_tempgrow_mean_h<-OSR_tempgrow_mean_h<-c()
ssd_tempgrow_h<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_temp_h))
max_yrs <- 20

for (i in 1:nrow(climat_temp_h)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_temp_h[i,1],
                           ztempgrow=climat_temp_h[i,2],
                           zpptdorm=climat_temp_h[i,3],
                           ztempdorm=climat_temp_h[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_tempgrow_mean_h[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_tempgrow_mean_2sex_h[i] <- lambda_run$lambdatracker[max_yrs]
  SR_tempgrow_mean_h[i] <- lambda_run$SRtracker[max_yrs]
  OSR_tempgrow_mean_h[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_tempgrow_h[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_temp_h$tempgrow_seq_h,lambda_tempgrow_mean_h,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_temp_h$tempgrow_seq_h,lambda_tempgrow_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
pptdorm_seq_h <- seq(min(zclim_past$zpptdorm),max(zclim_past$zpptdorm),length.out=20)
climat_pptdorm_h<-data.frame(pptgrow_seq_h=rep(mean(zclim_past$zpptgrow),length(pptdorm_seq_h)),tempgrow_seq_h=rep(mean(zclim_past$ztempgrow),length(pptdorm_seq_h)),pptdorm_seq_h=pptdorm_seq_h,tempdorm_seq_h=rep(mean(zclim_past$ztempdorm),length(pptdorm_seq_h)))

lambda_pptdorm_mean_h<-lambda_pptdorm_mean_2sex_h<-SR_pptdorm_mean_h<-OSR_pptdorm_mean_h<-c()
ssd_pptdorm_h<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_pptdorm_h))
max_yrs <- 20

for (i in 1:nrow(climat_pptdorm_h)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_pptdorm_h[i,1],
                           ztempgrow=climat_pptdorm_h[i,2],
                           zpptdorm=climat_pptdorm_h[i,3],
                           ztempdorm=climat_pptdorm_h[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pptdorm_mean_h [i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pptdorm_mean_2sex_h [i] <- lambda_run$lambdatracker[max_yrs]
  SR_pptdorm_mean_h[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pptdorm_mean_h[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pptdorm_h[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_pptdorm_h$pptdorm_seq_h,lambda_pptdorm_mean_h,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation of the dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_pptdorm_h$pptdorm_seq_h,lambda_pptdorm_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```

### Temperature of the dormant season
```{r}
tempdorm_seq_h <- seq(min(zclim_past$ztempdorm),max(zclim_past$ztempdorm),length.out=20)
climat_tempdorm_h<-data.frame(pptgrow_seq_h=rep(mean(zclim_past$zpptgrow),length(tempdorm_seq_h)),tempgrow_seq_extend=rep(mean(zclim_past$ztempgrow),length(tempdorm_seq_h)),pptdorm_seq_extend=rep(mean(zclim_past$zpptdorm),length(tempdorm_seq_h)),tempdorm_seq_h=tempdorm_seq_h)

lambda_tempdorm_mean_h<-lambda_tempdorm_mean_2sex_h<-SR_tempdorm_mean_h<-OSR_tempdorm_mean_h<-c()
ssd_tempdorm_h<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_tempdorm_h))
max_yrs <- 20

for (i in 1:nrow(climat_tempdorm_h)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_tempdorm_h[i,1],
                           ztempgrow=climat_tempdorm_h[i,2],
                           zpptdorm=climat_tempdorm_h[i,3],
                           ztempdorm=climat_tempdorm_h[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_tempdorm_mean_h[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_tempdorm_mean_2sex_h[i] <- lambda_run$lambdatracker[max_yrs]
  SR_tempdorm_mean_h[i] <- lambda_run$SRtracker[max_yrs]
  OSR_tempdorm_mean_h[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_tempdorm_h[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_tempdorm_h$tempdorm_seq_h,lambda_tempdorm_mean_h,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature of the dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_tempdorm_h$tempdorm_seq_h,lambda_tempdorm_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
```
...
# Precipitation of the growing season
```{r}
pptgrow_seq_MR45 <- seq(min(zclim_miroc45$zpptgrow),max(zclim_miroc45$zpptgrow),length.out=20)
climat_ppt_MR45<-data.frame(pptgrow_seq_MR45=pptgrow_seq_MR45,tempgrow_seq_MR45=rep(mean(zclim_miroc45$ztempgrow),length(pptgrow_seq_MR45)),pptdorm_seq_MR45=rep(mean(zclim_miroc45$zpptdorm),length(pptgrow_seq_MR45)),tempdorm_seq_MR45=rep(mean(zclim_miroc45$ztempdorm),length(pptgrow_seq_MR45)))

lambda_pptgrow_MR45<-lambda_pptgrow_MR45_2sex<-SR_pptgrow_MR45<-OSR_pptgrow_MR45<-c()
ssd_pptgrow_MR45<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_ppt_MR45))
max_yrs <- 20

for (i in 1:nrow(climat_ppt_MR45)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_ppt_MR45[i,1],
                           ztempgrow=climat_ppt_MR45[i,2],
                           zpptdorm=climat_ppt_MR45[i,3],
                           ztempdorm=climat_ppt_MR45[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pptgrow_MR45[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pptgrow_MR45_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_pptgrow_MR45[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pptgrow_MR45[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pptgrow_MR45[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_ppt_MR45$pptgrow_seq_MR45,lambda_pptgrow_MR45,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt_MR45$pptgrow_seq_MR45,lambda_pptgrow_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
temgrow_seq_MR45 <- seq(min(zclim_miroc45$ztempgrow),max(zclim_miroc45$ztempgrow),length.out=20)
climat_temp_MR45<-data.frame(pptgrow_seq_MR45=rep(mean(zclim_miroc45$zpptgrow),length(temgrow_seq_MR45)),tempgrow_seq_MR45=temgrow_seq_MR45,pptdorm_seq_MR45=rep(mean(zclim_miroc45$zpptdorm),length(temgrow_seq_MR45)),tempdorm_seq_MR45=rep(mean(zclim_miroc45$ztempdorm),length(temgrow_seq_MR45)))

lambda_tempgrow_MR45<-lambda_tempgrow_MR45_2sex<-SR_tempgrow_MR45<-OSR_tempgrow_MR45<-c()
ssd_tempgrow_MR45<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_temp_MR45))
max_yrs <- 20

for (i in 1:nrow(climat_temp_MR45)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_temp_MR45[i,1],
                           ztempgrow=climat_temp_MR45[i,2],
                           zpptdorm=climat_temp_MR45[i,3],
                           ztempdorm=climat_temp_MR45[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_tempgrow_MR45[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_tempgrow_MR45_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_tempgrow_MR45[i] <- lambda_run$SRtracker[max_yrs]
  OSR_tempgrow_MR45[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_tempgrow_MR45[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_temp_MR45$tempgrow_seq_MR45,lambda_tempgrow_MR45,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_temp_MR45$tempgrow_seq_MR45,lambda_tempgrow_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
pptdorm_seq_MR45 <- seq(min(zclim_miroc45$zpptdorm),max(zclim_miroc45$zpptdorm),length.out=20)
climat_pptdorm_MR45<-data.frame(pptgrow_seq_MR45=rep(mean(zclim_miroc45$zpptgrow),length(pptdorm_seq_MR45)),tempgrow_seq_MR45=rep(mean(zclim_miroc45$ztempgrow),length(pptdorm_seq_MR45)),pptdorm_seq_MR45=pptdorm_seq_MR45,tempdorm_seq_MR45=rep(mean(zclim_miroc45$ztempdorm),length(pptdorm_seq_MR45)))

lambda_pptdorm_MR45<-lambda_pptdorm_MR45_2sex<-SR_pptdorm_MR45<-OSR_pptdorm_MR45<-c()
ssd_pptdorm_MR45<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_pptdorm_MR45))
max_yrs <- 20

for (i in 1:nrow(climat_pptdorm_MR45)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_pptdorm_MR45[i,1],
                           ztempgrow=climat_pptdorm_MR45[i,2],
                           zpptdorm=climat_pptdorm_MR45[i,3],
                           ztempdorm=climat_pptdorm_MR45[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pptdorm_MR45[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pptdorm_MR45_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_pptdorm_MR45[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pptdorm_MR45[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pptdorm_MR45[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_pptdorm_MR45$pptdorm_seq_MR45,lambda_pptdorm_MR45,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation of dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_pptdorm_MR45$pptdorm_seq_MR45,lambda_pptdorm_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
temdorm_seq_MR45 <- seq(min(zclim_miroc45$ztempdorm),max(zclim_miroc45$ztempdorm),length.out=20)

climat_tempdorm_MR45<-data.frame(pptgrow_seq_MR45=rep(mean(zclim_miroc45$zpptgrow),length(temdorm_seq_MR45)),tempgrow_seq_MR45=rep(mean(zclim_miroc45$ztempgrow),length(temdorm_seq_MR45)),pptdorm_seq_MR45=rep(mean(zclim_miroc45$zpptdorm),length(temgrow_seq_MR45)),tempdorm_seq_MR45=temdorm_seq_MR45)

lambda_tempdorm_MR45<-lambda_tempdorm_MR45_2sex<-SR_tempdorm_MR45<-OSR_tempdorm_MR45<-c()
ssd_tempdorm_MR45<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_temp_MR45))
max_yrs <- 20

for (i in 1:nrow(climat_tempdorm_MR45)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_tempdorm_MR45[i,1],
                           ztempgrow=climat_tempdorm_MR45[i,2],
                           zpptdorm=climat_tempdorm_MR45[i,3],
                           ztempdorm=climat_tempdorm_MR45[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_tempdorm_MR45[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_tempdorm_MR45_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_tempdorm_MR45[i] <- lambda_run$SRtracker[max_yrs]
  OSR_tempdorm_MR45[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_tempdorm_MR45[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_tempdorm_MR45$tempdorm_seq_MR45,lambda_tempdorm_MR45,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature of dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_tempdorm_MR45$tempdorm_seq_MR45,lambda_tempdorm_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
# Precipitation of the growing season
```{r}
pptgrow_seq_MR85 <- seq(min(zclim_miroc85$zpptgrow),max(zclim_miroc85$zpptgrow),length.out=20)
climat_ppt_MR85<-data.frame(pptgrow_seq_MR85=pptgrow_seq_MR85,tempgrow_seq_MR85=rep(mean(zclim_miroc85$ztempgrow),length(pptgrow_seq_MR85)),pptdorm_seq_MR85=rep(mean(zclim_miroc85$zpptdorm),length(pptgrow_seq_MR85)),tempdorm_seq_MR85=rep(mean(zclim_miroc85$ztempdorm),length(pptgrow_seq_MR85)))

lambda_pptgrow_MR85<-lambda_pptgrow_MR85_2sex<-SR_pptgrow_MR85<-OSR_pptgrow_MR85<-c()
ssd_pptgrow_MR85<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_ppt_MR85))
max_yrs <- 20

for (i in 1:nrow(climat_ppt_MR85)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_ppt_MR85[i,1],
                           ztempgrow=climat_ppt_MR85[i,2],
                           zpptdorm=climat_ppt_MR85[i,3],
                           ztempdorm=climat_ppt_MR85[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pptgrow_MR85[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pptgrow_MR85_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_pptgrow_MR85[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pptgrow_MR85[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pptgrow_MR85[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_ppt_MR85$pptgrow_seq_MR85,lambda_pptgrow_MR85,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt_MR85$pptgrow_seq_MR85,lambda_pptgrow_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
tempgrow_seq_MR85 <- seq(min(zclim_miroc85$ztempgrow),max(zclim_miroc85$ztempgrow),length.out=20)
# tempgrow_seq_acc45 <- seq(min(tas_future_ACC_45_growing$tas_mean),max(tas_future_ACC_45_growing$tas_mean),length.out=20)

climat_temp_MR85<-data.frame(pptgrow_seq_MR85=rep(mean(zclim_miroc85$zpptgrow),length(tempgrow_seq_MR85)),tempgrow_seq_MR85=tempgrow_seq_MR85,pptdorm_seq_MR85=rep(mean(zclim_miroc85$zpptdorm),length(tempgrow_seq_MR85)),tempdorm_seq_MR85=rep(mean(zclim_miroc85$ztempdorm),length(tempgrow_seq_MR85)))

lambda_tempgrow_MR85<-lambda_tempgrow_MR85_2sex<-SR_tempgrow_MR85<-OSR_tempgrow_MR85<-c()
ssd_tempgrow_MR85<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_temp_MR85))
max_yrs <- 20

for (i in 1:nrow(climat_temp_MR85)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_temp_MR85[i,1],
                           ztempgrow=climat_temp_MR85[i,2],
                           zpptdorm=climat_temp_MR85[i,3],
                           ztempdorm=climat_temp_MR85[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_tempgrow_MR85[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_tempgrow_MR85_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_tempgrow_MR85[i] <- lambda_run$SRtracker[max_yrs]
  OSR_tempgrow_MR85[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_tempgrow_MR85[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_temp_MR85$tempgrow_seq_MR85,lambda_tempgrow_MR85,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_temp_MR85$tempgrow_seq_MR85,lambda_tempgrow_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
# Precipitation of the dormant season
```{r}
pptdorm_seq_MR85 <- seq(min(zclim_miroc85$zpptdorm),max(zclim_miroc85$zpptdorm),length.out=20)
climat_pptdorm_MR85<-data.frame(pptgrow_seq_MR85=rep(mean(zclim_miroc85$zpptgrow),length(pptdorm_seq_MR85)),tempgrow_seq_MR85=rep(mean(zclim_miroc85$ztempgrow),length(pptdorm_seq_MR85)),pptdorm_seq_MR85=pptdorm_seq_MR85,tempdorm_seq_MR85=rep(mean(zclim_miroc85$ztempdorm),length(pptgrow_seq_MR85)))

lambda_pptdorm_MR85<-lambda_pptdorm_MR85_2sex<-SR_pptdorm_MR85<-OSR_pptdorm_MR85<-c()
ssd_pptdorm_MR85<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_pptdorm_MR85))
max_yrs <- 20

for (i in 1:nrow(climat_pptdorm_MR85)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_pptdorm_MR85[i,1],
                           ztempgrow=climat_pptdorm_MR85[i,2],
                           zpptdorm=climat_pptdorm_MR85[i,3],
                           ztempdorm=climat_pptdorm_MR85[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pptdorm_MR85[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pptdorm_MR85_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_pptdorm_MR85[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pptdorm_MR85[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pptdorm_MR85[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(climat_pptdorm_MR85$pptdorm_seq_MR85,lambda_pptdorm_MR85,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation of dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_pptdorm_MR85$pptdorm_seq_MR85,lambda_pptdorm_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
## extend the climatic  to bounds and beyond of observed distribution
## note that length of this vector has a big effect on how fast/slow the following code runs
temdorm_seq_MR85 <- seq(min(zclim_miroc85$ztempdorm),max(zclim_miroc85$ztempdorm),length.out=20)
# tempgrow_seq_acc45 <- seq(min(tas_future_ACC_45_growing$tas_mean),max(tas_future_ACC_45_growing$tas_mean),length.out=20)

climat_tempdorm_MR85<-data.frame(pptgrow_seq_MR85=rep(mean(zclim_miroc85$zpptgrow),length(temdorm_seq_MR85)),tempgrow_seq_MR85=rep(mean(zclim_miroc85$ztempgrow),length(temdorm_seq_MR85)),pptdorm_seq_MR85=rep(mean(zclim_miroc85$zpptdorm),length(temdorm_seq_MR85)),tempdorm_seq_MR85=temdorm_seq_MR85)

lambda_tempdorm_MR85<-lambda_tempdorm_MR85_2sex<-SR_tempdorm_MR85<-OSR_tempdorm_MR85<-c()
ssd_tempdorm_MR85<-matrix(NA,(F_params$max_size+1)*2,nrow(climat_tempdorm_MR85))
max_yrs <- 20

for (i in 1:nrow(climat_tempdorm_MR85)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=climat_tempdorm_MR85[i,1],
                           ztempgrow=climat_tempdorm_MR85[i,2],
                           zpptdorm=climat_tempdorm_MR85[i,3],
                           ztempdorm=climat_tempdorm_MR85[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_tempdorm_MR85[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_tempdorm_MR85_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_tempdorm_MR85[i] <- lambda_run$SRtracker[max_yrs]
  OSR_tempdorm_MR85[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_tempdorm_MR85[,i] <- lambda_run$n0
}
...
```{r}
par(mar=c(5,5,1,1))
plot(temdorm_seq_MR85,lambda_tempdorm_MR85,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature of dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(temdorm_seq_MR85,lambda_tempdorm_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
#print figure
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/lambda_present_MIROC5_scaled.pdf",height = 8,width = 8,useDingbats = F)
layout(mat = layout.matrix,
...
# par(mar=c(5,5,1,1))
plot(climat_ppt_h$pptgrow_seq_h*sd(clim_past$pptgrow)+mean(clim_past$pptgrow),lambda_pptgrow_mean_h,type="l",lwd=2,col=cbPalette[1],xlab="",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt_h$pptgrow_seq_h*sd(clim_past$pptgrow)+mean(clim_past$pptgrow),lambda_pptgrow_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_temp_h$tempgrow_seq_h*sd(clim_past$tempgrow)+mean(clim_past$tempgrow),lambda_tempgrow_mean_h,type="l",lwd=2,col=cbPalette[1],xlab="",ylab="",cex.lab=1.3)
lines(climat_temp_h$tempgrow_seq_h*sd(clim_past$tempgrow)+mean(clim_past$tempgrow),lambda_tempgrow_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_pptdorm_h$pptdorm_seq_h*sd(clim_past$pptdorm)+mean(clim_past$pptdorm),lambda_pptdorm_mean_h,type="l",lwd=2,col=cbPalette[1],xlab="",ylab="",cex.lab=1.3)
lines(climat_pptdorm_h$pptdorm_seq_h*sd(clim_past$pptdorm)+mean(clim_past$pptdorm),lambda_pptdorm_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_tempdorm_h$tempdorm_seq_h*sd(clim_past$tempdorm) + mean(clim_past$tempdorm),lambda_tempdorm_mean_h,type="l",lwd=2,col=cbPalette[1],
     xlab="",ylab="",cex.lab=1.3)
lines(climat_tempdorm_h$tempdorm_seq_h*sd(clim_past$tempdorm) + mean(clim_past$tempdorm),lambda_tempdorm_mean_2sex_h,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_ppt$pptgrow_seq_extend*sd(clim_current$pptgrow)+ mean(clim_current$pptgrow),lambda_pptgrow_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="",ylab=expression(paste(lambda)),cex.lab=1.3)
title("A",adj=0)    
lines(climat_ppt$pptgrow_seq_extend*sd(clim_current$pptgrow)+ mean(clim_current$pptgrow),lambda_pptgrow_mean_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_temp$tempgrow_seq_extend*sd(clim_current$tempgrow)+mean(clim_current$tempgrow),lambda_tempgrow_mean,type="l",lwd=2,col=cbPalette[1],xlab="",ylab="",cex.lab=1.3)
lines(climat_temp$tempgrow_seq_extend*sd(clim_current$tempgrow)+mean(clim_current$tempgrow),lambda_tempgrow_mean_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_pptdorm$pptdorm_seq_extend*sd(clim_current$pptdorm)+mean(clim_current$pptdorm),lambda_pptdorm_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="",ylab="",cex.lab=1.3)
lines(climat_pptdorm$pptdorm_seq_extend*sd(clim_current$pptdorm)+mean(clim_current$pptdorm),lambda_pptdorm_mean_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_tempdorm$tempdorm_seq_extend*sd(clim_current$tempdorm)+mean(clim_current$tempdorm),lambda_tempdorm_mean,type="l",lwd=2,col=cbPalette[1],
     xlab="",ylab="",cex.lab=1.3)
lines(climat_tempdorm$tempdorm_seq_extend*sd(clim_current$tempdorm)+mean(clim_current$tempdorm),lambda_tempdorm_mean_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_ppt_MR45$pptgrow_seq_MR45*sd(clim_miroc45$pptgrow)+mean(clim_miroc45$pptgrow),lambda_pptgrow_MR45,type="l",lwd=2,col=cbPalette[1],
     xlab="",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt_MR45$pptgrow_seq_MR45*sd(clim_miroc45$pptgrow)+mean(clim_miroc45$pptgrow),lambda_pptgrow_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_temp_MR45$tempgrow_seq_MR45*sd(clim_miroc45$tempgrow)+mean(clim_miroc45$tempgrow),lambda_tempgrow_MR45,type="l",lwd=2,col=cbPalette[1],xlab="",ylab="",cex.lab=1.3)
lines(climat_temp_MR45$tempgrow_seq_MR45*sd(clim_miroc45$tempgrow)+mean(clim_miroc45$tempgrow),lambda_tempgrow_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_pptdorm_MR45$pptdorm_seq_MR45*sd(clim_miroc45$pptdorm)+mean(clim_miroc45$pptdorm),lambda_pptdorm_MR45,type="l",lwd=2,col=cbPalette[1],xlab="",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_pptdorm_MR45$pptdorm_seq_MR45*sd(clim_miroc45$pptdorm)+mean(clim_miroc45$pptdorm),lambda_pptdorm_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_tempdorm_MR45$tempdorm_seq_MR45*sd(clim_miroc45$tempdorm)+mean(clim_miroc45$tempdorm),lambda_tempdorm_MR45,type="l",lwd=2,col=cbPalette[1],
     xlab="",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_tempdorm_MR45$tempdorm_seq_MR45*sd(clim_miroc45$tempdorm)+mean(clim_miroc45$tempdorm),lambda_tempdorm_MR45_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_ppt_MR85$pptgrow_seq_MR85*sd(clim_miroc85$pptgrow)+mean(clim_miroc85$pptgrow),lambda_pptgrow_MR85,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_ppt_MR85$pptgrow_seq_MR85*sd(clim_miroc85$pptgrow)+mean(clim_miroc85$pptgrow),lambda_pptgrow_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_temp_MR85$tempgrow_seq_MR85*sd(clim_miroc85$tempgrow)+mean(clim_miroc85$tempgrow),lambda_tempgrow_MR85,type="l",lwd=2,col=cbPalette[1],xlab="Temperature growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_temp_MR85$tempgrow_seq_MR85*sd(clim_miroc85$tempgrow)+mean(clim_miroc85$tempgrow),lambda_tempgrow_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(climat_pptdorm_MR85$pptdorm_seq_MR85*sd(clim_miroc85$pptdorm)+mean(clim_miroc85$pptdorm),lambda_pptdorm_MR85,type="l",lwd=2,col=cbPalette[1],xlab="Precipitation of dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(climat_pptdorm_MR85$pptdorm_seq_MR85*sd(clim_miroc85$pptdorm)+mean(clim_miroc85$pptdorm),lambda_pptdorm_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
# legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)

# par(mar=c(5,5,1,1))
plot(temdorm_seq_MR85*sd(clim_miroc85$tempdorm)+mean(clim_miroc85$tempdorm),lambda_tempdorm_MR85,type="l",lwd=2,col=cbPalette[1],
     xlab="Temperature of dormant season",ylab=expression(paste(lambda)),cex.lab=1.3)
lines(temdorm_seq_MR85*sd(clim_miroc85$tempdorm)+mean(clim_miroc85$tempdorm),lambda_tempdorm_MR85_2sex,type="l",lwd=2,col=cbPalette[2])
legend("topright",legend=c("Female-dominant","Two-sex"),lwd=2,col =cbPalette ,bty="n",cex=1)
...
# Simulation to test parallelization

## Without parallelization
```{r}
A <- system.time({
  lambda_tempgrow_mean_2sex_simulation <- foreach(i=1:nrow(climat_temp), 
               .combine = 'c'
  ) %do% (lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 surv_perturb=0,
                                 flow_perturb=0,
...
```{r}
plot(climat_ppt$pptgrow_seq_extend,lambda_tempgrow_mean_2sex_simulation,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
```
...
```{r}
P <- system.time({
  lambda_tempgrow_mean_2sex_simulation_parallel <- foreach(i=1:nrow(climat_temp), 
               .combine = 'c'
...
```{r}
plot(climat_ppt$pptgrow_seq_extend,lambda_tempgrow_mean_2sex_simulation_parallel,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
```

```{r}
clim_current$pptgrow
plot(climat_ppt$pptgrow_seq_extend*sd(clim_current$pptgrow)+mean(clim_current$pptgrow),lambda_tempgrow_mean_2sex_simulation_parallel,type="l",lwd=2,col=cbPalette[1],
     xlab="Precipitation growing season",ylab=expression(paste(lambda)),cex.lab=1.3)
```
...
tempgrow_seq_extend<-climat_temp[,2]
pptdorm_seq_extend<-climat_temp[,3]
tempdorm_seq_extend<-climat_temp[,4]


lambda_tempgrow_post <- Fdom_lambda_tempgrow <- SR_tempgrow_post <- OSR_tempgrow_post <- matrix(NA,nrow=n_post_draws,ncol=length(tempgrow_seq_extend))
lambda_tempgrow_post_rfx <- Fdom_lambda_tempgrow_rfx <- SR_tempgrow_post_rfx <- OSR_tempgrow_post_rfx <- matrix(NA,nrow=n_post_draws,ncol=length(tempgrow_seq_extend))
...
for(p in 1:n_post_draws){
 ...
 for(l in 1:length(tempgrow_seq_extend)){
   ...
   lambda_run <- lambdaSim_delay(F_params=F_params,M_params=M_params,pptgrow=pptgrow_seq_extend[l],tempgrow=tempgrow_seq_extend[l],pptdorm=pptdorm_seq_extend[l],tempdorm=tempdorm_seq_extend[l],rfx=rfx_fun(),max.yrs=max_yrs)
   lambda_tempgrow_post[p,l] <- lambda_run$lambdatracker[max_yrs]
   SR_tempgrow_post[p,l] <- lambda_run$SRtracker[max_yrs]
   OSR_tempgrow_post[p,l] <- lambda_run$OSRtracker[max_yrs]

   #Estimation+process error
   lambda_run_rfx <- lambdaSim_delay(F_params=F_params,M_params=M_params,pptgrow=pptgrow_seq_extend[l],tempgrow=tempgrow_seq_extend[l],pptdorm=pptdorm_seq_extend[l],tempdorm=tempdorm_seq_extend[l],rfx=rfx,max.yrs=max_yrs)
   lambda_tempgrow_post_rfx[p,l] <- lambda_run_rfx$lambdatracker[max_yrs]
   SR_tempgrow_post_rfx[p,l] <- lambda_run_rfx$SRtracker[max_yrs]
   OSR_tempgrow_post_rfx[p,l] <- lambda_run_rfx$OSRtracker[max_yrs]
   print(c(p,l))
 }
}
## write output as a list
lambda_tempgrow_post_list <- list(lambda_tempgrow_post=lambda_tempgrow_post,SR_tempgrow_post=SR_tempgrow_post,OSR_tempgrow_post=OSR_tempgrow_post,
                             lambda_tempgrow_post_rfx=lambda_tempgrow_post_rfx,SR_tempgrow_post_rfx=SR_tempgrow_post_rfx,OSR_tempgrow_post_rfx=OSR_temp_post_rfx)
# write_rds(lambda_temp_post_list,paste0(dir,"/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/Forecasting Models output/lambda_tempgrow_post_list.rds"))

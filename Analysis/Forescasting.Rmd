...
garden_osr <- poar.clim_seasonal %>% 
  dplyr::select(year, site, Block,Code,Sex,zpptgrow,zpptdorm,ztempgrow,ztempdorm,flowerN_t1) %>% 
  group_by(year,zpptgrow,zpptdorm,ztempgrow,ztempdorm,site,Block,Code, Sex) %>% 
...
garden_osr_poolyr <- poar.clim_seasonal %>% 
  dplyr::select(site,Block,Code, Sex, zpptgrow,zpptdorm,ztempgrow, ztempdorm,flowerN_t1) %>% 
  group_by(zpptgrow,zpptdorm,ztempgrow,ztempdorm, site,Block,Code, Sex) %>% 
  summarise(total_panicles = sum(flowerN_t1,na.rm=T)) %>% 
  spread(key=Sex,value=total_panicles) %>% 
  na.omit %>% 
  mutate( site= site %>% as.factor %>% as.numeric,
...
garden_sr <- poar.clim_seasonal %>% 
  dplyr::select( year, site,Block,Code, Sex, zpptgrow,zpptdorm,ztempgrow, ztempdorm,surv_t1) %>% 
  group_by(year, zpptgrow,zpptdorm,ztempgrow, ztempdorm, site,Block,Code, Sex) %>% 
...
garden_sr_poolyr <- poar.clim_seasonal %>%
  dplyr::select(site,Block,Code, Sex, zpptgrow, zpptdorm, ztempgrow, ztempdorm, surv_t1) %>% 
  group_by(zpptgrow,zpptdorm,ztempgrow, ztempdorm, site,Block,Code, Sex) %>% 
  summarise(total_plants = sum(surv_t1,na.rm=T)) %>% 
  spread(key=Sex,value=total_plants) %>% 
  rename(fem = `F`,
...
garden_osr_dat <- list(y = garden_osr_poolyr$fem_pan,
                       ...
                       n_blocks = garden_osr_poolyr$Block %>% n_distinct,
                       n_sites   = garden_osr_poolyr$site %>% n_distinct,
                       n_sources =  garden_osr_poolyr$source %>% n_distinct,
                       site   = garden_osr_poolyr$site,
                       source =  garden_osr_poolyr$source,
                        block  = garden_osr_poolyr$Block,
...
coef_garden_osr <- rstan::extract(fit_garden_osr_tempgrow,pars = quote_bare(b0,b_tempgrow,b_pptgrow,b_pptdorm,b_tempgrow,b_tempdorm,b_pptgrow2,b_pptdorm2,b_tempgrow2,b_tempdorm2,b_tempdormpptdorm,b_tempgrowpptgrow))
```

```{r}
posterior_osr <- as.array(fit_garden_osr)
color_scheme_set("orange")
osr_pptgrow<-mcmc_intervals(posterior_osr, pars = quote_bare(b0,b_tempgrow,b_pptgrow,b_pptdorm,b_tempgrow,b_tempdorm,b_pptgrow2,b_pptdorm2,b_tempgrow2,b_tempdorm2,b_tempdormpptdorm,b_tempgrowpptgrow)) + 
  ggplot2::scale_y_discrete(limits = b0,b_tempgrow,b_pptgrow,b_pptdorm,b_tempgrow,b_tempdorm,b_pptgrow2,b_pptdorm2,b_tempgrow2,b_tempdorm2,b_tempdormpptdorm,b_tempgrowpptgrow,
                            labels=c("b0"="b0",
                                     "b_pptgrow"="pptgrow",
                                     "b_tempgrow"="tempgrow",
                                     "b_pptdorm"="pptdorm",

...
```{r}
nichedormant<-read.csv("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/Forecasting Models output/data_dormant_niche.csv")
prob_lambda_dorm_post<-c()
for(l in 1:ncol(nichedormant)){
  prob_lambda_dorm_post[l]<-mean(nichedormant[,l]>1,na.rm=T)
}

all_pptdorm_seq<-seq(min(data_niche$zpptdorm),max(data_niche$zpptdorm),length.out=30)
all_tempdorm_seq<-seq(min(data_niche$ztempdorm),max(data_niche$ztempdorm),length.out=30)
...
contour(all_pptdorm_seq,all_tempdorm_seq,mtrx_dorm,add=T,labcex=1,col="black") 
...
all_pptgrow_seq<-seq(min(data_niche$zpptgrow),max(data_niche$zpptgrow),length.out=30)
all_tempgrow_seq<-seq(min(data_niche$ztempgrow),max(data_niche$ztempgrow),length.out=30)
...
contour(all_pptgrow_seq,all_tempgrow_seq,mtrx_grow,add=T,labcex=1,col="black") 

...
plot(source,add=T,pch = 23,col="black",bg="grey",cex =1)
legend(-106.5, 28.5, 
...
geo_prlambda_past_fd_twosex$lambda_diff<-geo_prlambda_past_fd_twosex$Prlambda.x-geo_prlambda_past_fd_twosex$Prlambda.y
```
...
(lambda_map_Prpast_diff <- ggplot()+
  ...
  annotate(geom="text", x=-100, y=41, label="Past",
              color="grey",size = 3)+
  scale_fill_gradientn(
    name = expression(paste(Delta," (E-A)")),
...
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Updated/Fig_geoPrlambdacmc.pdf", Fig_geoPrlambdacmc, width = 10, height = 8)
```
...
Fig_geoPrlambdaacc<-ggarrange(lambda_map_Prpast, lambda_map_Prcurrent, lambda_map_Pracc45, lambda_map_Pracc85,lambda_map_Prpast_fd, lambda_map_Prcurrent_fd,lambda_map_acc45_fd,lambda_map_acc85_fd,ncol=4,nrow=2,
                                        common.legend = TRUE)
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Updated/Fig_geoPrlambdaacc_.pdf", Fig_geoPrlambdaacc, width = 10, height = 8)
```
...
```{r,eval=FALSE}
#pull out parameter posterior distributions
pred_ORS_tempgrow <- rstan::extract(fit_garden_osr_tempgrow, pars = c("pred"))$pred
pred_ORS_tempdorm <- rstan::extract(fit_garden_osr_tempdorm, pars = c("pred"))$pred
pred_ORS_pptgrow <- rstan::extract(fit_garden_osr_pptgrow, pars = c("pred"))$pred
pred_ORS_pptdorm <- rstan::extract(fit_garden_osr_pptdorm, pars = c("pred"))$pred

#draw 500 random samples from the joint posterior
n_post_draws <- 500
post_draws <- sample.int(dim(pred_ORS_tempgrow)[1], n_post_draws)
#set up simulation output
y_osr_tempgrow_sim <- matrix(NA,n_post_draws,length(garden_osr_dat$y))
y_osr_tempdorm_sim <- matrix(NA,n_post_draws,length(garden_osr_dat$y))
y_osr_pptgrow_sim <- matrix(NA,n_post_draws,length(garden_osr_dat$y))
y_osr_pptdorm_sim <- matrix(NA,n_post_draws,length(garden_osr_dat$y))
#loop over the posterior and generate new observations
for(i in 1:n_post_draws){
  print(i)
  y_osr_tempgrow_sim[i,] <- rbinom(n=length(garden_osr_dat$y), size=1, prob = invlogit(pred_ORS_tempgrow[i,]))
  y_osr_tempdorm_sim[i,] <- rbinom(n=length(garden_osr_dat$y), size=1, prob = invlogit(pred_ORS_tempdorm[i,]))
  y_osr_pptgrow_sim[i,] <- rbinom(n=length(garden_osr_dat$y), size=1, prob = invlogit(pred_ORS_pptgrow[i,]))
  y_osr_pptdorm_sim[i,] <- rbinom(n=length(garden_osr_dat$y), size=1, prob = invlogit(pred_ORS_pptdorm[i,]))
}
...
```{r,eval=FALSE}
#plot ppc overlay
ppc_dens_overlay(garden_osr_dat$y, y_osr_tempgrow_sim)+
  xlab("Temperature (grow.seson)")+ylab("Density")+
  ggtitle(("Survival"))+theme(legend.position = "none")->ppc_tempgrow

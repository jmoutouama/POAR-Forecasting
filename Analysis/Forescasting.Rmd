...
```{r}
data_past_present_85<-rbind(zclim_past,zclim_current,data_site_85)
```

```{r}
lambda_pp85<-lambda_pp85_2sex<-SR_pp85<-OSR_pp85<-c()
ssd_pp85<-matrix(NA,(F_params$max_size+1)*2,nrow(data_past_present_85))
max_yrs <- 30

for (i in 1:nrow(data_past_present_85)) {
    #linear model for comparison
  mat <- megamatrix_delay( F_params=F_params,
                           M_params=M_params,
                           twosex=F,
                           ...
                           fert_perturb=0,
                           viab_perturb=0,
                           zpptgrow=data_past_present_85[i,1],
                           ztempgrow=data_past_present_85[i,2],
                           zpptdorm=data_past_present_85[i,3],
                           ztempdorm=data_past_present_85[i,4],
                           rfx=rfx_fun())$MEGAmat
  lambda_pp85[i] <- popbio::lambda(mat)
  # 2-sex model
  lambda_run <- lambdaSim_delay( F_params=F_params,
                                 M_params=M_params,
                                 grow_perturb=0,
                                 ...
                                 rfx = rfx_fun(),
                                 max.yrs = max_yrs)
  lambda_pp85_2sex[i] <- lambda_run$lambdatracker[max_yrs]
  SR_pp85[i] <- lambda_run$SRtracker[max_yrs]
  OSR_pp85[i] <- lambda_run$OSRtracker[max_yrs]
  ssd_pp85[,i] <- lambda_run$n0
}
```

```{r}
Time<-c(rep("Past",14),rep("Present",14),rep("Future",14))
datltre85<-data.frame(data_past_present_85,lambda=lambda_pp85_2sex,Time=Time)
```

```{r}
ou85<-tuneRF(y=datltre85$lambda, x=as.data.frame(datltre85[,-c(5,6)]),stepFactor = 1,improve = 0.02)
plot(ou85)
modrf85<-randomForest::randomForest(y=datltre85$lambda, x=as.data.frame(datltre85[,-c(5,6)]),ntree=500,importance=TRUE,mtry=1)
plot(modrf85$rsq,type="l")
randomForest::varImpPlot(modrf85,type = 1,scale = FALSE,main="")
```

```{r}
datLTRE<-rbind(imp45,imp85)
datLTRE$RCP<-c("4.5","4.5","4.5","4.5","8.5","8.5","8.5","8.5")
(Fig_LTRE <- ggplot(data=datLTRE, aes(x=vars, y=Importance, fill=RCP)) +
geom_bar(stat="identity", color="black", position=position_dodge())+
  labs(x="")+
  scale_x_discrete(
    breaks = c("tempgrow","tempdorm","pptgrow","pptdorm"),
    labels = c("Temperature (grow.season)", "Temperature (dorm.season)","Precipitation (grow.season)","Precipitation (dorm.season)"))+
  coord_flip()+
  theme_bw()+
...
```{r}
# all_pptgrow<-c(data_past_present_45$zpptgrow,data_past_present_45$zpptgrow,data_past_present_45$zpptgrow,data_past_present_45$zpptgrow)
all_pptgrow_seq <- seq(min(data_past_present_45$zpptgrow),max(data_past_present_45$zpptgrow),length.out=30)
...
```{r}
# all_tempgrow<-c(data_past_present_45$ztempgrow,data_past_present_45$ztempgrow,zclim_miroc45$ztempgrow,zclim_miroc85$ztempgrow)
all_tempgrow_seq <- seq(min(data_past_present_45$ztempgrow),max(data_past_present_45$ztempgrow),length.out=20)
...
all_pptdorm_seq <- seq(min(data_past_present_45$zpptdorm),max(data_past_present_45$zpptdorm),length.out=20)
...
```{r}
all_tempdorm_seq <- seq(min(data_past_present_45$ztempdorm),max(data_past_present_45$ztempdorm),length.out=30)
...
```{r}
rect_4.5_pptgrow <- data.frame(xmin = c((min(zclim_past$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (min(zclim_current$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (min(data_site_45$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow))), xmax = c((max(zclim_past$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (max(zclim_current$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (max(data_site_45$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow))),
                      Time = c("Past", "Present", "RCP4.5"))
...
figcombo1<-ggplot() +
  ...
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_4.5_pptgrow, inherit.aes = FALSE) +
  xlab(" Precipitation (grow. season)") +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  ...
  theme(legend.position="none",
        ...
        axis.title.x = element_text(size = 9,color="black"),
        axis.title.y = element_text(size = 9,color="black"))+
  scale_fill_manual(values = c("green", "yellow", "red"),name="") 


rect_4.5_tempgrow <- data.frame(xmin = c((min(zclim_past$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (min(zclim_current$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (min(data_site_45$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow))), xmax = c((max(zclim_past$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (max(zclim_current$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (max(data_site_45$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow))),
                      Time = c("Past", "Present", "RCP4.5"))
figcombo2<-ggplot() +
  ...
  geom_hline(yintercept = 1, lty=2,color = "black")+
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_4.5_tempgrow, inherit.aes = FALSE) +
  xlab(" Temperature (grow. season)") +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  scale_y_continuous(breaks = c(0,1,2,3,4,5,6,7,8))+
  ggtitle("")+
...
rect_4.5_pptdorm <- data.frame(xmin = c((min(zclim_past$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (min(zclim_current$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (min(data_site_45$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm))), xmax = c((max(zclim_past$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (max(zclim_current$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (max(data_site_45$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm))),
                      Time = c("Past", "Present", "RCP4.5"))
...
figcombo3<-ggplot() +
  ...
  xlab(" Precipitation (dorm. season)") +
 geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_4.5_pptdorm, inherit.aes = FALSE) +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  ggtitle("")+
  theme_bw() +
  theme(legend.position="none",
...
rect_4.5_tempdorm <- data.frame(xmin = c((min(zclim_past$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (min(zclim_current$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (min(data_site_45$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm))), xmax = c((max(zclim_past$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (max(zclim_current$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (max(data_site_45$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm))),
                      Time = c("Past", "Present", "RCP4.5"))
...
figcombo4<-ggplot() +
  ...
  geom_hline(yintercept = 1, lty=2,color = "black")+
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_4.5_tempdorm, inherit.aes = FALSE) +
  xlab(" Temperature (grow. season)") +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  ggtitle("")+
  theme_bw() +
...
```{r}
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/lambdapredcited45.pdf",width=5,height=5,useDingbats = F)
...
```{r}
rect_8.5_pptgrow <- data.frame(xmin = c((min(zclim_past$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (min(zclim_current$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (min(data_site_85$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow))), xmax = c((max(zclim_past$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (max(zclim_current$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow)), (max(data_site_85$zpptgrow)*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow))),
                      Time = c("Past", "Present", "RCP8.5"))

figcombo851<-ggplot() +
  geom_line(aes(all_pptgrow_seq85*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow),lambda_pptgrow_all85), col = "#000000",size=1) +
  geom_line(aes(all_pptgrow_seq85*sd(poar_2015_2016$pptgrow)+mean(poar_2015_2016$pptgrow),lambda_pptgrow_all_2sex85), col = "#000000",size=1,lty=2) +
  geom_hline(yintercept = 1, lty=2,color = "black")+
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_8.5_pptgrow, inherit.aes = FALSE) +
  xlab(" Precipitation (grow. season)") +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  theme_bw() +
  theme(legend.position="none",
...
rect_8.5_tempgrow <- data.frame(xmin = c((min(zclim_past$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (min(zclim_current$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (min(data_site_85$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow))), xmax = c((max(zclim_past$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (max(zclim_current$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow)), (max(data_site_85$ztempgrow)*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow))),
                      Time = c("Past", "Present", "RCP8.5"))
figcombo852<-ggplot() +
  geom_line(aes(all_tempgrow_seq85*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow),lambda_tempgrow_all85), col = "#000000",size=1)+
  geom_line(aes(all_tempgrow_seq85*sd(poar_2015_2016$tempgrow)+mean(poar_2015_2016$tempgrow),lambda_tempgrow_all_2sex85), col = "#000000",size=1,lty=2) +
  geom_hline(yintercept = 1, lty=2,color = "black")+
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_8.5_tempgrow, inherit.aes = FALSE) +
  xlab(" Temperature (grow. season)") +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  scale_y_continuous(breaks = c(0,1,2,3,4,5,6,7,8))+
  ggtitle("")+
...
rect_8.5_pptdorm <- data.frame(xmin = c((min(zclim_past$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (min(zclim_current$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (min(data_site_85$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm))), xmax = c((max(zclim_past$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (max(zclim_current$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm)), (max(data_site_85$zpptdorm)*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm))),
                      Time = c("Past", "Present", "RCP8.5"))

figcombo853<-ggplot() +
  geom_line(aes(all_pptdorm_seq85*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm),lambda_pptdorm_all85), col = "#000000",size=1)+
  geom_line(aes(all_pptdorm_seq85*sd(poar_2015_2016$pptdorm)+mean(poar_2015_2016$pptdorm),lambda_pptdorm_all_2sex85), col = "#000000",size=1,lty=2) +
  geom_hline(yintercept = 1, lty=2,color = "black")+
  xlab(" Precipitation (dorm. season)") +
 geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_8.5_pptdorm, inherit.aes = FALSE) +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  ggtitle("")+
  theme_bw() +
  theme(legend.position="none",
...
rect_8.5_tempdorm <- data.frame(xmin = c((min(zclim_past$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (min(zclim_current$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (min(data_site_85$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm))), xmax = c((max(zclim_past$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (max(zclim_current$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm)), (max(data_site_85$ztempdorm)*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm))),
                      Time = c("Past", "Present", "RCP8.5"))

figcombo854<-ggplot() +
  geom_line(aes(all_tempdorm_seq85*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm),lambda_tempdorm_all85), col = "#000000",size=1) +
  geom_line(aes(all_tempdorm_seq85*sd(poar_2015_2016$tempdorm)+mean(poar_2015_2016$tempdorm),lambda_tempdorm_all_2sex85), col = "#000000",size=1,lty=2) +
  geom_hline(yintercept = 1, lty=2,color = "black")+
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = Time), 
                alpha = 0.2, data = rect_8.5_tempdorm, inherit.aes = FALSE) +
  xlab(" Temperature (grow. season)") +
  ylab(expression(paste("Population growth rate, ", lambda))) +
  ggtitle("")+
  theme_bw() +
...
```{r}
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/lambdapredcited85.pdf",width=5,height=5,useDingbats = F)
(figurecombo1 <- ggarrange(figcombo851, figcombo852,figcombo853,figcombo854,
                     hjust = -2, labels = c("A", "B","C","D"), common.legend = TRUE,legend="bottom" ,font.label = list(size = 10, color = "black", face = "plain", family = NULL), vjust = 2, widths = c(2, 2), heights = 0.5, align = "hv",
                     ncol = 2, nrow = 2
))
dev.off()

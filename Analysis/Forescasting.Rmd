...
#print figure
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/lambda_past_present_future.pdf",height = 9,width = 10,useDingbats = F)
layout(mat = layout.matrix,
       heights = rep(c(2, 0.25),4), # Heights of the two rows
       widths = c(2, 2, 2))
# layout.show(8)
...
legend(650, 7.5, legend=c("Female dominant", "Two-sex"),
       col=c("#E69F00", "#999999"), lty=1:1, cex=1,bty = "n",lwd=4,seg.len =0.5:0.5)
...
mtext(" RCPs",side=2,line=3,cex=1.2)
mtext("Precipitation (grow. season)",side=1,line=3,cex=1.2)
segments(x0 = rect_pptgrow[1,1],y0 = 2, x1 = rect_pptgrow[1,2],y1 = 2, col= colors[1]) 
segments(x0 = rect_pptgrow[2,1],y0 = 1.5, x1 = rect_pptgrow[2,2],y1 =1.5, col = colors[2]) 
segments(x0 = rect_pptgrow[3,1],y0 = 1, x1 = rect_pptgrow[3,2],y1 =1, col = colors[3]) 
segments(x0 = rect_pptgrow[4,1],y0 = 0.5, x1 = rect_pptgrow[4,2],y1 = 0.5, col = colors[4]) 
...
segments(x0 = rect_tempgrow[1,1],y0 = 2, x1 = rect_tempgrow[1,2],y1 = 2, col= colors[1]) 
segments(x0 = rect_tempgrow[2,1],y0 = 1.5, x1 = rect_tempgrow[2,2],y1 =1.5, col = colors[2]) 
segments(x0 = rect_tempgrow[3,1],y0 = 1, x1 = rect_tempgrow[3,2],y1 =1, col = colors[3]) 
segments(x0 = rect_tempgrow[4,1],y0 = 0.5, x1 = rect_tempgrow[4,2],y1 = 0.5, col = colors[4]) 
...
mtext(" RCPs",side=2,line=3,cex=1.2)
segments(x0 = rect_pptdorm[1,1], x1 = rect_pptdorm[1,2], y0 = 2, y1 = 2, col = colors[1])
segments(x0 = rect_pptdorm[2,1], x1 = rect_pptdorm[2,2], y0 = 1.5, y1 = 1.5, col = colors[2]) 
segments(x0 = rect_pptdorm[3,1], x1 = rect_pptdorm[3,2], y0 = 1, y1 = 1, col = colors[3])
segments(x0 = rect_pptdorm[4,1], x1 = rect_pptdorm[4,2], y0 = 0.5, y1 = 0.5, col = colors[4])
...
segments(x0 = rect_tempdorm[1,1], x1 = rect_tempdorm[1,2], y0 = 2, y1 = 2, col = colors[1])
segments(x0 = rect_tempdorm[2,1], x1 = rect_tempdorm[2,2], y0 = 1.5, y1 = 1.5, col = colors[2]) 
segments(x0 = rect_tempdorm[3,1], x1 = rect_tempdorm[3,2], y0 = 1, y1 = 1, col = colors[3])
segments(x0 = rect_tempdorm[4,1], x1 = rect_tempdorm[4,2], y0 = 0.5, y1 = 0.5, col = colors[4])
legend(32, 2.5, legend=c("Past", "Present", "RCP4.5","RCP8.5"),
       col=colors, lty=1:1, seg.len =0.5:0.5,bty ="n",cex=0.5)
...
for(l in 1:nrow(pivot_lambda_45)){
  prob_lambda_45_post[l]<-mean(pivot_lambda_45[l,]>1,na.rm=T)
}
...
for(l in 1:nrow(pivot_lambda_85)){
  prob_lambda_85_post[l]<-mean(pivot_lambda_85[l,]>1,na.rm=T)
}
...
```{r}
Fig_geoPrlambdaprojection<-ggarrange(lambda_map_Prpast, lambda_map_Prcurrent, lambda_map_Prmiroc45, lambda_map_Prmiroc85,common.legend = TRUE)
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Fig_geoPrlambdaprojection.pdf", Fig_geoPrlambdaprojection, width = 5, height = 7)
```
...
```{r}
Fig_geoPrlambdacmcprojection<-ggarrange(lambda_map_Prpast, lambda_map_Prcurrent, lambda_map_Prcmc45, lambda_map_Prcmc85,common.legend = TRUE)
ggsave("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/Fig_geoPrlambdaprojectioncmc.pdf", Fig_geoPrlambdacmcprojection, width = 5, height = 7)
```

### Female dominant model

```{r}
geo_lambbacurrent_fd <- read.csv("https://www.dropbox.com/scl/fi/4bpcjgf80kva9gex0vnc4/lambda_current.csv?rlkey=uwpwd14ma5vp1jq9iwix07c0g&dl=1", stringsAsFactors = F) 
...
geo_lambda_miroc45_fd<-read.csv("https://www.dropbox.com/scl/fi/4bm84k4lbnkz3duup2c22/lambda_miroc45.csv?rlkey=iz793zg59fn92un2zl331awzc&dl=1", stringsAsFactors = F)
geo_lambba_miroc85_fd <- read.csv("https://www.dropbox.com/scl/fi/jpeywkhbiyaap3qwmrdmn/datamiroc85fd.csv?rlkey=9yim1d2m748f2wsrenxtetk94&dl=1", stringsAsFactors = F)
...
for(l in 1:ncol(lambbapast_fd)){
  prob_lambda_past_fd[l]<-mean(lambbapast_fd[,l]>1,na.rm=T)
}
...
for(l in 1:ncol(lambbacurrent_fd)){
  prob_lambda_current_fd[l]<-mean(lambbacurrent_fd[,l]>1,na.rm=T)
}
...
for(l in 1:ncol(geo_lambba_cmc85_fd)){
  prob_lambda_cmc85_fd[l]<-mean(geo_lambba_cmc85_fd[,l]>1,na.rm=T)
}
...
lam_prob_acc45_fd<-data.frame(climate_acc_45_values[,9:10],Prlambda=prob_lambda_cmc45_fd)
```
...
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/niche.pdf",width=5,height=7,useDingbats = F)
par(mar=c(5,5,1,1),mfrow=c(2,1))
fields::image.plot(all_pptdorm_seq,all_tempdorm_seq,mtrx_dorm,col=topo.colors(100),xlab="Precipitation dormant",ylab="Temperature dormant",main="") 
...
text(x=x_past_d,y=y_past_d,"+",col="black")
text(x=x_miroc45_d,y=y_miroc45_d,"o",col="black",cex=0.8)
text(x=x_miroc85_d,y=y_miroc85_d,".",col="black",cex=3.5)
fields::image.plot(all_pptgrow_seq,all_tempgrow_seq,mtrx_grow,col=topo.colors(100),xlab="Precipitation growing",ylab="Temperature growing",main="") 
contour(all_pptgrow_seq,all_tempgrow_seq,mtrx_grow,add=T,labcex=0.75,col="black") 
mtext("B",side = 3, adj = 0,cex=1.4)
# title(main="B" ,adj=0,cex.main=1.4,font=1)
text(x=x_current_g,y=y_current_g,"*",col="black",cex=1.5)
text(x=x_past_g,y=y_past_g,"+",col="black")
text(x=x_miroc45_g,y=y_miroc45_g,"o",col="black",cex=0.8)
text(x=x_miroc85_g,y=y_miroc85_g,".",col="black",cex=3.5)
dev.off()
```
...
pdf("/Users/jm200/Library/CloudStorage/Dropbox/Miller Lab/github/POAR-Forecasting/Manuscript/Figures/niche_female_dominant.pdf",width=5,height=7,useDingbats = F)
par(mar=c(5,5,1,1),mfrow=c(2,1))
fields::image.plot(all_pptdorm_seq,all_tempdorm_seq,mtrx_dorm_fd,col=topo.colors(100),xlab="Precipitation dormant",ylab="Temperature dormant",main="") 
...
text(x=x_miroc45_d,y=y_miroc45_d,"o",col="black",cex=0.8)
text(x=x_miroc85_d,y=y_miroc85_d,".",col="black",cex=3.5)
fields::image.plot(all_pptgrow_seq,all_tempgrow_seq,mtrx_grow_fd,col=topo.colors(100),xlab="Precipitation growing",ylab="Temperature growing",main="") 

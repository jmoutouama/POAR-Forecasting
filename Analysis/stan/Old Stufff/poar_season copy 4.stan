...
model {
  ...
  bpptgrow2sex_p ~ normal(0, 0.25);  
  bpptdorm2sex_p ~ normal(0, 0.25); 
  btempgrow2sex_p ~ normal(0, 0.25);  
  btempdorm2sex_p ~ normal(0, 0.25); 
  bpptgrow2_p ~ normal(0, 0.25); 
  bpptdorm2_p ~ normal(0, 0.25); 
  btempgrow2_p ~ normal(0, 0.25); 
  btempdorm2_p ~ normal(0, 0.25); 
  block_tau_p ~ inv_gamma(0.25, 0.25);
  for (i in 1:n_blocks_p){
    block_rfx_p[i] ~ normal(0, block_tau_p);
  }
  source_tau_p ~ inv_gamma(0.25, 0.25);
  for (i in 1:n_sources){
    source_rfx_p[i] ~ normal(0, source_tau_p);
  }
  site_tau_p ~ inv_gamma(0.25, 0.25);
  for (i in 1:n_sites){

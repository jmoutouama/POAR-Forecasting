...
transformed parameters {
  ...
  for(isurv in 1:n_s){
    predS[isurv] = b0_s + 
                //main effects
                bsize_s * size_s[isurv] + bsex_s * male_s[isurv] + bpptgrow_s * pptgrow_s[isurv]  + btempgrow_s * tempgrow_s[isurv]   +
                //2-way interactions
  ...
  for(igrow in 1:n_g){
    predG[igrow] = exp(b0_g + 
                //main effects
                bsize_g * size_g[igrow] + bsex_g * male_g[igrow] + bpptgrow_g * pptgrow_g[igrow] + btempgrow_g * tempgrow_g[igrow]   +
                //2-way interactions
  ...
  for(iflow in 1:n_f){
    predF[iflow] = b0_f + 
                //main effects
                bsize_f * size_f[iflow] + bsex_f * male_f[iflow] + bpptgrow_f * pptgrow_f[iflow] + btempgrow_f * tempgrow_f[iflow]   +
                //2-way interactions
  ...
  for(ipan in 1:n_p){
    predP[ipan] = b0_p + 
                //main effects
                bsize_p * size_p[ipan] + bsex_p * male_p[ipan] + bpptgrow_p * pptgrow_p[ipan] + btempgrow_p * tempgrow_p[ipan]  +            
                //2-way interactions

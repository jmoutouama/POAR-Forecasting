...
model {
  ...
  for (i in 1:n_blocks_s){
    block_rfx_s[i] ~ normal(0, block_tau);
  }
  source_tau ~ inv_gamma(0.1, 0.1);
  for (i in 1:n_sources){
    source_rfx_s[i] ~ normal(0, source_tau);
  }
  site_tau~ inv_gamma(0.1, 0.1);
  for (i in 1:n_sites){
    site_rfx_s[i] ~ normal(0, site_tau);
  }
  ...
  for (i in 1:n_blocks_g){
    block_rfx_g[i] ~ normal(0, block_tau);
  }
  //source_tau_g ~ inv_gamma(0.1, 0.1);
  for (i in 1:n_sources){
    source_rfx_g[i] ~ normal(0, source_tau);
  }
  //site_tau_g ~ inv_gamma(0.1, 0.1);
  for (i in 1:n_sites){
    site_rfx_g[i] ~ normal(0, site_tau);
  }
  for(i in 1:n_g){
    theta[i] ~ ig(1, sigma);
  }
  ...
  for (i in 1:n_blocks_f){
    block_rfx_f[i] ~ normal(0, block_tau);
  }
  //source_tau_f ~ inv_gamma(0.1, 0.1);
  for (i in 1:n_sources){
    source_rfx_f[i] ~ normal(0, source_tau);
  }
  //site_tau_f ~ inv_gamma(0.1, 0.1);
  for (i in 1:n_sites){
    site_rfx_f[i] ~ normal(0, site_tau);
  }
  // fertility
  b0_p ~ normal(0, 0.2);    
  bsize_p ~ normal(0, 0.2);   
  bsex_p ~ normal(0, 0.2);   
  bpptgrow_p ~ normal(0, 0.2);  
  bpptdorm_p ~ normal(0, 0.2);  
  btempgrow_p ~ normal(0, 0.2);  
  btempdorm_p ~ normal(0, 0.2);
  bsizesex_p ~ normal(0, 0.2);
  btempdormpptdorm_p ~ normal(0,0.2);
  btempgrowpptgrow_p ~ normal(0, 0.2);
  bpptgrowsex_p ~ normal(0, 0.2);
  bpptdormsex_p ~ normal(0, 0.2);
  btempgrowsex_p ~ normal(0, 0.2);
  btempdormsex_p ~ normal(0, 0.2);
  btempdormpptdormsex_p ~ normal(0,0.2);
  btempgrowpptgrowsex_p ~ normal(0, 0.2);
  bpptgrow2sex_p ~ normal(0,0.2);  
  bpptdorm2sex_p ~ normal(0, 0.2); 
  btempgrow2sex_p ~ normal(0, 0.2);  
  btempdorm2sex_p ~ normal(0, 0.2); 
  bpptgrow2_p ~ normal(0, 0.2); 
  bpptdorm2_p ~ normal(0,0.2); 
  btempgrow2_p ~ normal(0, 0.2); 
  btempdorm2_p ~ normal(0, 0.2); 
  //block_tau_p ~ inv_gamma(0.1, 0.1);
  for (i in 1:n_blocks_p){
    block_rfx_p[i] ~ normal(0, block_tau);
  }
  //source_tau_p ~ inv_gamma(0.2, 0.2);
  for (i in 1:n_sources){
    source_rfx_p[i] ~ normal(0, source_tau);
  }
  //site_tau_p ~ inv_gamma(0.2, 0.2);
  for (i in 1:n_sites){
    site_rfx_p[i] ~ normal(0, site_tau);
  }

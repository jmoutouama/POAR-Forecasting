...
\tom{}{Rather than just describe climate, this would be a good place to say more about climate change, which I have added. All of this sets the stage for why this is an important system to forecast climate change responses. If you think it is better we could move this to Results, since it is based on the climate data you describe below. I would like to figure out why the time series figure showed no change in precipitation but the past-present-future normals show a big change.}
...
The linear predictor includes normally distributed random effects for block-to-block variation ($\phi \sim N(0,\ \sigma_{block})$), site to site variation ($\nu \sim N(0,\ \sigma_{site})$), and source-to-source variation that is related to the genetic provenance of the transplants used to establish the common garden ($\rho \sim N(0,\ \sigma_{source})$).

...
Accounting for such a niche differentiation between males and females within a population is a long-standing challenge in accurately predicting which sex will successfully track environmental change and how this will impact population viability and range shifts \citep{jones1999sex,gissi2023exploring}. 
...
\tom{Females and males may respond differently to climate change, especially in species where there is sexual niche differentiation \citep{gissi2023exploring,gissi2023sex,hultine2016climate}.}{This paragraph is really good but notice that the topic sentence (and much that follows) is largely redundant with the first paragraph. I would suggest creating clearer distinction between paragraphs.} 
...
The geographic range of most species may be limited by climatic factors, including temperature, precipitation. 
Any substantial changes in the magnitude of these climatic factors in a given location could impact population viability, with implications for range shifts based on which regions become more or less suitable \citep{davis2001range, pease1989model}. 
Forecasting range shifts for dioecious species is complicated by the potential for each sex to respond differently to climate variation.
Populations in which males are rare under current climatic conditions could experience low reproductive success due to sperm or pollen limitation that may lead to population decline in response to climate change that favors females \citep{eberhart2017sex}.
...
\tom{Our ability to track the impact of climate change on the population dynamics of dioecious plants and the implication of such impact on range shift depends on our ability to build mechanistic models that take into account the spatial and temporal context in which sex specific response to climate change affects population viability \citep{davis2001range,evans2016towards,czachura2020demographic}.}{This is a great topic sentence but the paragraph does not really expand upon this point about mechanistic models. The next few sentences do not really say anything new. I think it would be stronger to discuss the value and challenges of mechanistic models for species' range shifts.}
...
Secondly, we downloaded monthly temperature and precipitation from Chelsa to describe current (2015–2016) climate conditions \citep{karger2017climatologies}.
These climate data were used as covariates in vital rate regressions, which allowed us to forecast and back cast the effect of climate change on population dynamics and map species’ distributions under future and past climate change. 
We prefer temperature and precipitation because they capture the most the climate in the study region. 
...
To forecast the effect of future climate on species population dynamics, we downloaded climatic projection data (2017-20100) from four general circulation models (GCMs) selected from the Coupled Model Intercomparison Project Phase 5 (CMIP5). 
These GCMs are MIROC5, ACCESS1-3, CESM1-BGC, CMCC-CM  and were downloaded from chelsa \citep{sanderson2015representative}.
...
\renewcommand{\thefigure}{S\arabic{figure}}
  ...
  \begin{figure}[H]
		\centering
		\includegraphics[scale = 0.5]{Figures/PPC22_10_23.pdf}

...
The common experiment was installed at 14 sites across a climatic gradient (FigX).
...
To understand the impact of climate change on species niche shifts, we estimated the conditional probability of the population viability being more than one across two environmental axes \citep{diez2014probabilistic}.
In this study, the environmental axes are temperature and precipitation of the dormant season or temperature and precipitation of the growing season. 
...
We estimated the predicted response to population growth rate (population fitness) to seasonal climate gradients using a model assuming a female dominant model and another model using the two sexes. 
...
Furthermore, population fitness was maximized between 23 and 17 degrees Celsius and decreases to zero just beyond 32 degrees Celsius during the growing season.
Similarly population fitness was maximized between 13 and 17 degrees Celsius and decreases to zero just beyond 20 degrees Celsius during the growing season.

...
Both model versions predict that future climate change will alter population viability and will induce a poleward shift of niche suitability beyond current northern limits.
However, the magnitude of niche shift was underestimated by the female-dominant model, because females have broader temperature tolerance than males but become mate-limited under female-biased sex ratios.
...
Forecasting range shifts for dioecious species (most animals and \tom{ca. 7\% of plant species}{I think the 7\% estimate needs a citation.}) is complicated by the potential for sexual niche differentiation, i.e. distinct responses of females and males to shared climate drivers \citep{hultine2016climate,morrison2016causes}. 
...
\tom{Accounting for sexual niche differentiation is a long-standing challenge in accurately predicting which sex will successfully track environmental change and how this will impact population viability and range shifts \citep{jones1999sex,gissi2023exploring}. 
...
Although the response of species to climate warming is an urgent and active area of research, few studies have disentangled the interaction between sex and climate drivers to understand their combined effects on population dynamics and range shifts, despite calls for such an approach \citep{hultine2016climate,gissi2023exploring}.}{This is an important paragraph and I think these sentences could be sharpened and reorganized to more effectively set up the knowledge gaps. Let's talk through this in person.}
...
\tom{In this study,}{Most of my comments are about how you needed more changes to accommodate the PNAS format. Because the methods come at the end, readers need to be given enough information here to understand the results that will follow. SO my suggestion is to think about making this paragraph maximally informative with respect to the methods, so that the results make sense.} we combined geographically-distributed common garden experiments, hierarchical Bayesian statistical modeling, two-sex population projection modeling, and climate back-casting and forecasting to understand demographic responses to climate change and their implications for past, present, and future range dynamics. 
Our work focused on the dioecious plant Texas bluegrass (\textit{Poa arachnifera}), which is distributed along environmental gradients in the south-central U.S. corresponding to variation in temperature across latitude and precipitation across longitude (Fig. S1). 
This region has experienced rapid climate warming since 1900 and warming is projected to continue through the end of the century (Fig. \ref{fig:study_design}, \tom{Fig. S2}{I think this is the better figure to show climate wamring. But you are missing two panels from it.}). 
...
Developing a rigorous forecast for the implications of future climate change requires that we transition from implicit to explicit treatment of multiple climate drivers, \tom{as we do here.}{Since the methods now come after the results, you need to do more to help readers understand the results that will follow. For example, this would be a place to introduce the idea of growing and dormant season climate.}
...
Fig. S4 and  S5 visualize the magnitude of sexual divergence in demography across niche space, revealing that female advantages in flowering and \tom{panicle production}{This is not correct for panicles. } were greatest at both high and low \tom{growing season temperature extremes}{Notice that there is nothing to prepare readers for seasonal climate.}. 
...
OSR was most female-biased (up to 80\% female) at extreme values of temperature, especially growing season temperature (Fig. S6, Fig. S7), consistent with the female reproductive advantage at temperature extremes seen in the vital rate data (Fig. S4, Fig. S5). 
...
Putting all vital rates together in \tom{the  matrix projection model (MPM)}{Again, notice how readers will not be prepared for this.} framework reveals how climate shapes fitness variation across niche dimensions and geographic space, and how accounting for sex structure modifies these inferences. 
...
These visual trends are supported by LTRE decomposition indicating that variation in fitness across climatic conditions is most strongly driven by responses to growing and dormant season temperature, with weaker interactive effects of precipitation that modulate the effects of temperature (Fig. S9). 
...
Female-dominant and two-sex models diverged most strongly in regions of niche space that favored strongly female-biased operational sex ratios \tom{(Fig. S12)}{I think you mean S13 here, and I suspect the top panel is not correct.}.
This suggests mate limitation as the biological mechanism underlying model differences. 
\tom{The two-sex model accounts for feedbacks between OSR and female fertility, with reduced seed viability at OSR exceeding $\sim$ 75\% female panicles (Fig. S14).}{Readers should understand that this came from a separate experiment.}
...
The predicted geographic niche is more expansive than the \tom{GBIF occurrences}{YOu have not mentioned anything about GBIF occurrences, so readers will not understand this.}, particularly at southern, western, and eastern edges, suggesting some degree of range disequilibrium (e.g., due to dispersal limitation), geographic bias in occurrence records, and/or model mis-specification. 
...
The predicted poleward niche shift is consistent across different global circulation models (Fig. S15, Fig. S16, Fig. S17). 
...
These hotspots are generally regions of predicted female bias in the operational sex ratio \tom{(Fig. S18, Fig. S19, Fig. S20, Fig. S21). }{This is a long list of figures for one simple statement -- if you are going to reference all these figures you need to provide some context for why.}
The strongest contrast between the two models is in the pessimistic climate change scenario (RCP8.5), where the female-dominant model over-predicts population viability by as much as 20\% across much of the region (Fig. S18) and thus under-estimates the magnitude of a potential range shift. 
...
The OSR of Texas bluegrass across its range is projected to be ca. 75\% female panicles, on average, by end of century under RCP8.5, an increase from ca. 60\% female under projections for past and current conditions (Fig. S19,  Fig. S20). 
The more optimistic climate change scenario (RCP4.5) predicts an intermediate shift in OSR, with hotspots of change at northern and southern range edges becoming strongly female-biased but most of the range remaining near current levels of 60\% female \tom{(Fig. S21, Fig. S22, Fig. S23, Fig. S24). }{Could all of these climate model figures be combined into a single multi-panel figure?}
...
For example, studies suggested that species that are active \tom{during the growing season such as cool grass species}{This does not really make sense. Every species is active during the growing season, because every species can have a different growing season.} can have delayed phenology in response to global warming, particularly if temperatures rise above their physiological tolerances \citep{ williams2015life}.
...
\tom{}{This seems like a long methods section for PNAS, and it is not clear to me how you decided what should go here vs the supplement. Are there any limits on this section for the main paper?}
...
On the other hand, precipitation has increased over the past century for much of the region but is forecasted to decline back to early-20th century levels \tom{(Fig. S2).}{Notice that figure 2 is missing these panels.} 
...
\begin{align}\label{eq:viab_MPM}
	v(\mathbf{F_{t}},\mathbf{M_{t}}) = v_{0} * \left[ 1 - \Phi(x) \right]
\end{align}
...
\tom{During peer review, our code (Stan and R) is available at \url{https://github.com/jmoutouama/POAR-Forecasting}. }{Make sure that your github repo has a clear ReadMe file that will make it easy for reviewers to find and understand the code.}

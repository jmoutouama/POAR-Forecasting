...
\revise{Sex biases are ubiquitous in nature and central to biology. 
However, most forecasts of population viability and range shifts overlook the complexity of sex structure, and thus the potential for females and males to differ in their sensitivity to climate drivers.
 We} used a combination of  demographic data collected from a  common garden experiment, climate change forecasts, and mathematical models to demonstrate that accounting for only one sex could underestimate the impact of climate change on dioecious species, particularly in regions of their range that are biased toward one sex. 
...
\revise{This is good news from a conservation standpoint because most studies on dioecious species rely on the female-dominant approach \citep{caswell1986two}}.
...
\revise{Here, the female bias was not a direct response to climate, such as temperature-dependent sex determination seen in some organisms like invertebrates and reptiles \citep{korpelainen1990sex}. 
Instead, bias in sex ratio resulted from climate-induced changes in vital rates that accumulate throughout species life cycle.}
...
%This finding  is consistent with a recent synthesis suggesting that the dormant season matters for plants \citep{evers2021lagged}. 
One probable explanation for the importance of the dormant season in our study system is that Texas, Oklahoma and Kansas often experience a very high  temperature during   the summer which corresponds to the dormant season  (Fig. \ref{fig:study_design}).
Therefore, an increase in temperature during the dormant season could reduce water storage in the soil, negatively affecting plant survival and growth.
Furthermore, high levels of precipitation that occurred during our study were typically associated with tropical storms during the late-summer dormant season or extreme weather systems during the spring growing season. 
For example, the wettest conditions we observed were linked to the 2015 flooding and tornado outbreak across much of Texas and Oklahoma. 
Therefore, the negative effects of high precipitation were likely driven by waterlogged soils and disturbances associated with these extreme events.}
...
Biological sex is genetically based and the birth (seed) sex ratio is 1:1 \citep{jacobs2000grasses}. 
...
\revise{In \emph{Poa arachnifera}, unfertilized ovules lead to the production of non-viable seeds and only seed viability is dependent on  the operational sex ratio \citep{compagnoni2017can}.}
...
We used the climate-dependent vital rate regressions estimated above, combined with \revise{additional data on seed number, seed viability and seed germination to} build female-dominant and two-sex versions of a climate-explicit matrix projection model (MPMs) structured by the discrete state variables size (number of tillers) and sex.
...
This experiment also provided  parameters for seed number per panicle ($d$) and germination rate ($m$).

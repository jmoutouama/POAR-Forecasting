...
\tom{Bayesian mixed effect models, describing how each vital rate varies as a function of sex, size, and climate covariates (precipitation and temperature of growing and dormant seasons), revealed the demographic response of Texas bluegrass to climate drivers across common garden sites and years, and identified demographic differences between the sexes. 
Regression coefficients related to sex and/or sex:size interactions were significantly non-zero (95\% credible intervals excluding zero), \revise{particularly for probability of flowering (Fig. S3), suggesting sexual divergence in demography whereby females had an advantage over males (Fig. \ref{fig:vital_rates}).} 
...
Furthermore, there were significant effects of climate covariates and interactions between sex and one or more climate variables (Fig. S3), indicating sexual niche divergence in response to shared climate drivers.  
Fig. S4 and S5 visualize the magnitude of sexual divergence in demography across niche space, revealing that female advantages in survival and flowering  were greatest at both high and low growing season temperature extremes. }{I think this paragraph needs to be edited more carefully so that our statements about statistical support are unambiguous -- following the reviewer's comment. I made some changes but I am skipping over this for now.}
...
We next projected the climatic niche onto geographic space (Texas, Oklahoma and Kansas) to examine how suitable niche conditions for Texas bluegrass are shifting due to climate change, \revise{considering relatively ``optimistic''(RCP4.5)  and ``pessimistic''(RCP8.5) scenarios (Fig. \ref{fig:geoprojcmc}).}\tom{}{I think I added this text last time so I flagged it as ``revise''. I recommend you do this for all changes, even those that are not in direct response to a reviewer comment. That helps give the impression that the manuscript has changed from our original submission.} 
...
This suggests a particularly important role of sex-structured modeling for threatened and endangered species, as conservation biologists have already recognized \tom{\citep{jenouvrier2012effects}. }{If you are only going to cite this one paper I would use ``e.g.,''}
...
\revise{Here, the female bias was not a direct response to climate, such as temperature-dependent sex determination seen in some organisms like invertebrates and reptiles \citep{korpelainen1990sex}\tom{}{You could also cite something more directly about climate change, such as Janzen 1994 PNAS}. 
...
High dormant-season temperatures could reduce water storage in the soil, negatively affecting plant survival and growth. 
\tom{We also found some negative effects of high growing-season precipitation, which may reflect extreme weather systems during the spring growing season. }{I changed the section on precip because these effects were only negative in the growing season.}
...
\caption{Sex specific demographic response to climate across species range.
			...
			The fitted lines were estimated using only one climate covariate, while the other covariates and size were held constant. %TOM: say what size this is plotted for.

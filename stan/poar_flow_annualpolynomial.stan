...
transformed parameters {
  ...
  for(iflow in 1:n_f){
    predF[iflow] = b0_f + 
                ...
                bppttemp_f * temp_f[iflow] * ppt_f[iflow] + 
                bcvppttemp_f * cvppt_f[iflow] * temp_f[iflow] + 
               
                //3-way interaction
                bppttempsex_f * temp_f[iflow] * ppt_f[iflow] * male_f[iflow] +
                bcvppttempsex_f * temp_f[iflow] * cvppt_f[iflow] * male_f[iflow] +

                //polynomial 2

                bppt2_f * pow(ppt_f [iflow],2) + 
                btemp2_f * pow(temp_f [iflow],2) + 
                bcvppt2_f * pow(cvppt_f [iflow],2) + 
                bppt2sex_f * male_f [iflow] * pow(ppt_f[iflow],2) +
                btemp2sex_f * male_f [iflow] * pow(temp_f[iflow],2) +
                bcvppt2sex_f * male_f [iflow] * pow(cvppt_f[iflow],2) +
...
model {
  ...
  bcvppttemp_f ~ normal(0, 100);
  bcvppttempsex_f ~ normal(0, 100);
  bppttempsex_f ~ normal(0, 100);
  bppt2_f ~ normal(0, 100);
  btemp2_f ~ normal(0, 100);
  bcvppt2_f ~ normal(0, 100);
  bppt2sex_f ~ normal(0, 100);
  btemp2sex_f ~ normal(0, 100);
  bcvppt2sex_f ~ normal(0, 100);

...
transformed parameters {
  ...
  for(isurv in 1:n_s){
    predS[isurv] = b0_s + 
                ...
                bppt2_s * pow(ppt_s [isurv],2) + 
                btemp2_s * pow(temp_s [isurv],2) + 
                bcvppt2_s * pow(cvppt_s [isurv],2) + 
                bppt2sex_s * male_s [isurv] * pow(ppt_s[isurv],2) +
                btemp2sex_s * male_s [isurv] * pow(temp_s[isurv],2) +
                bcvppt2sex_s * male_s [isurv] * pow(cvppt_s[isurv],2) +
...
model {
  ...
  bppt2sex_s ~ normal(0, 100);
  btemp2sex_s ~ normal(0, 100);
  bcvppt2sex_s ~ normal(0, 100);
